...
//! chat data, isolated temp storage and loopback-only website. Workers globals
//! are initialized like the app; UNPEEL_HOME must point to the isolated QA home.
...
fn main() -> anyhow::Result<()> {
    let bcu = std::env::args().any(|arg| arg == "--bcu");
    ...
    gpui_platform::application().with_assets(icons::Assets).run(move |cx| {
        ...
        let window = cx.open_window(WindowOptions {
            ...
            ..Default::default()
        }, |_, cx| { let workers = cx.new(|cx| workers::model::WorkersModel::new(state.clone(), cx)); let monitor = cx.new(|cx| workers::resource_monitor::WorkersResourceMonitor::new(workers.clone(), cx)); cx.set_global(workers::resource_monitor::WorkersResourceGlobal { monitor }); let controller = cx.new(|cx| workers::menu_bar::WorkersMenuBarController::new(workers.clone(), cx)); cx.set_global(workers::menu_bar::WorkersMenuBarGlobal { controller }); cx.new(|cx| shell::Shell::new(state.clone(), boot, workers, cx)) }).unwrap();
        ...
        cx.spawn(async move |cx| {
            let run: anyhow::Result<()> = async {
                ...
                state.update(cx, |s, cx| {
                    ...
                    s.apply_transcript_frame(zeron_doc::TranscriptFrame::Reset { reset: entries }).unwrap(); cx.notify();
                });
                ...
                if !std::env::args().any(|arg| arg == "--resize-regression") { capture(&output, "browser-empty-dark")?; }
                #[cfg(target_os = "macos")]
                {
                    ...
                    pause(cx, 500).await;
                    if !std::env::args().any(|arg| arg == "--resize-regression") { capture(&output, "browser-preview-dark")?; }
                    if std::env::args().any(|arg| arg == "--resize-regression") {
                        ...
                        for width in [380.25, 640.5, 520.75] {
                            ...
                            anyhow::ensure!((layout-native).abs()<0.01 && (clip-native).abs()<0.01, "fractional resize left a gap: {layout}/{native}/{clip}");
                        }
                        std::fs::write(output.join("result.txt"), "PASS: native divider hit testing and fractional browser geometry\n")?;
                        return Ok(());
                    }
