...
| 1.3 Shell layout | done | Collapsible drag-resizable sidebar (208–400), full-height shared right utility column for Terminal/Changes (360–760, 52% cap), independent pane header, widths persisted to `ui-settings.json`. |
...
| 1.10 Terminal panel | done | Discoverable titlebar toggle + Cmd+J, named `Terminal ×` pane header, session-scoped tabs, lateral Terminal/Changes exclusivity, width drag, replay-then-tail streams, input coalescing, ANSI emulator (`ui/src/terminal/`). |
