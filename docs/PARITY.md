...
| 1.7 Composer | done | Send/Steer/Stop morph, compact↔expanded flip, per-Chat drafts, double-click select-all, indexed full-width scrollable `@`/`/` completion, target row above a bare new-Chat canvas, QuestionPanel, model/harness/trait/project/ref pickers, and image/text-file attachment intake with failure hand-back and preview. |
...
| 3.2 Sessions engine | done | Run journal on disk with crash recovery (aborted stamps), steering mailbox at step boundaries, doc hooks at boundaries, streamed part folding at STREAM_COMMIT_MS, idle reaper via `hibernation_candidates` / `idle_since_unix_ms` (`crates/workers-unpeel/src/lib.rs`), and stall watchdog. |
...
| Item | Status | Notes |
| --- | --- | --- |
| Token-usage display | done | Activity bridge tracking (`crates/workers-unpeel/src/activity_bridge.rs:1400`), token usage presentation in details sidebar (`crates/ui/src/details_sidebar/view.rs:1623`), rate-limit meters + Usage AgentEvent passthrough kept as specified. |
