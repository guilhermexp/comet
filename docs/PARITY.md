...
| 1.3 Shell layout | done | Collapsible drag-resizable sidebar (208–400), shared right utility pane for Terminal/Changes (360–760, 52% cap), header variants, widths persisted to `ui-settings.json`. |
...
| 1.10 Terminal panel | done | Discoverable titlebar toggle + Cmd+J, session-scoped tabs, lateral Terminal/Changes exclusivity, width drag, replay-then-tail streams, input coalescing, ANSI emulator (`ui/src/terminal/`). |
