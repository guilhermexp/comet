...
Chat. The empty tab lists its running services. **Open** navigates to
...
`WatchPreviews` resolves the Chat's current cwd on each catalog or workspace
change. On the viewing device it selects either the local services or the
advertised services for the Chat's owning device. Changing the active
...
restart while capturing the native UI. Screenshots/videos remain local QA artifacts. Use BCU for native interaction; the browser fixture accepts `--bcu` to leave control with BCU.
...
The embedded browser is available on macOS. Other platforms retain the external-browser fallback. This change does not replace the existing native HTML/Markdown document previews.
