...
|---|---|---|
| `src/chat_client.rs` + `src/chat_client/tests.rs` (cursor, checkpoint e recuperação causal em HTTP/WebSocket) | unit | `cargo test -p zeron-sync --features mock-server --lib chat_client` |
