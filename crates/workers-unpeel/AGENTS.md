...
- **`wait_for_status(completed)` casa evidência do episódio atual, não o processo.** O Worker vivo fica `state=running` / `activity=idle` depois do Stop (filhos MCP de longa duração). O predicado reusa Stop/`done`+unread + `WorkerCompletionEvidence` + generation/task-episode. O latch `acknowledged_completed_episode` mantém a conclusão observável depois do ACK só se os guards vivos ainda valem — blocked, geração nova, output crescendo ou episódio N-1 na mesma geração não completam. Idle sem evidência não é completed. `wait_until_matching` é o núcleo injetável; o teto de 4h não muda.
...
| `tests/parent_notifications.rs` (26) | integration | `--test parent_notifications` |
