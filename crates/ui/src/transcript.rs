...
impl Transcript {
    ...
    fn render_user_attachments(
        ...
        for (aix, att) in atts.iter().enumerate() {
            ...
            // only the unrefreshed echo carries `pending://` (queued flow) or
            // synthetic `pending/` (legacy flow) refs, so
            // the pair (pending path, upload in flight) is exactly "this image
            // is crossing the relay right now" (2026-08-18 user request).
            let uploading = (att.path.starts_with("pending/")
                || att.path.starts_with("pending://"))
