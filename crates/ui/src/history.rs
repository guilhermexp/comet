...
//!
//! Hosted in its own utility tab; selecting a commit opens a separate diff tab.
...
impl GitHistorySearchControl {
    ...
    fn blur(&mut self, cx: &mut Context<Self>) {
        // A result click blurs the input before selecting its row. Keep the
        // query so that this click cannot select a different, unfiltered row.
...
impl GitHistory {
    ...
    pub fn for_cwd(state: Entity<AppState>, cwd: String, cx: &mut Context<Self>) -> Self {
        let mut history = Self::new(state, cx);
        history.explicit_cwd = Some(cwd);
        history
    }

    fn context(&self, cx: &App) -> Option<(String, String, Option<String>)> {
        if let Some(cwd) = &self.explicit_cwd {
            return Some((format!("local|{cwd}"), cwd.clone(), None));
        }
...
mod tests {
    ...
    #[test]
    fn history_search_keeps_filtered_rows_when_focus_moves_to_a_result() {
        let mut app = gpui::TestApp::new();
        app.update(|cx| {
            Theme::install(crate::theme::Appearance::Dark, cx);
            crate::composer::init(cx);
        ...
        window.draw();
        window.update(|harness, window, cx| {
            harness.search.update(cx, |search, cx| {
                search.history.update(cx, |history, _| {
                    history.commits = vec![commit("other", &[]), commit("preview", &[])];
                    history.recompute_view();
                });
                search.expand(window, cx);
            });
        });
        window.update(|_, window, cx| window.simulate_next_frame(cx));
        window.draw();
        window.update(|harness, _, cx| {
            harness.search.update(cx, |search, cx| {
                search
                    .input
                    .update(cx, |input, cx| input.set_text("preview", cx));
            });
        });
        window.draw();
        // A result click moves focus away before its mouse-up handler runs.
        // Clearing the query here replaces the target with the unfiltered row.
        window.update(|harness, _, cx| {
            harness.search.update(cx, |search, cx| search.blur(cx));
        });
        window.draw();
        window.update(|harness, _, cx| {
            let search = harness.search.read(cx);
            assert_eq!(search.input.read(cx).text(), "preview");
            let history = search.history.read(cx);
            assert_eq!(history.search_query, "preview");
            assert_eq!(history.visible_commits.len(), 1);
            assert_eq!(history.visible_commits[0].sha, "preview");
        });
    }
