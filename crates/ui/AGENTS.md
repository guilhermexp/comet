...
- O gesto de captura (menu nativo de modo → `screencapture` → cancelamento) mora **uma vez** em `workers::session_gallery::pick_and_capture`; `Ok(None)` é cancelamento, não erro. Orchestrator e Workers só escolhem destino e o que fazer com o arquivo. E o botão de captura da titlebar renderiza uma vez por estado: overlay posicionado (`orchestrator_capture_right_offset`) com o pane aberto, in-row com ele fechado — duas cópias no mesmo frame duplicam os ids `orchestrator-capture`/`-menu`.
