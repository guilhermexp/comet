...
- **Prazo de transporte da bridge de Workers (`TOOL_CALL_TIMEOUT` = 900s) excede o teto de bloqueio da tool (`wait_for_status` = 120s)**: a folga deliberada de 780s evita que o transporte seja a falha primária e supera o piso de 60s para round-trip de IPC, serialização JSON-RPC e scheduling.
