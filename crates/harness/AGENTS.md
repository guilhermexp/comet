...
- **Normalização de tools do OMP**: `normalize_tool` mapeia `grep` para `ToolCall::Search` (com `pattern` e `path` opcional) e `glob` para `ToolCall::Glob` (extraindo o padrão do campo `path` do protocolo OMP, nunca de `pattern`).
