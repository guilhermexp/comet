...
*scenario:recap*)
  ...
  while [ "$#" -gt 0 ]; do
    case "$1" in
      --tools) shift; [ "$1" = "" ] && tools_off=true ;;
      --system-prompt) shift; case "$1" in "Summarize this conversation."*) system_set=true ;; esac ;;
      --strict-mcp-config) mcp_off=true ;;
      --dangerously-skip-permissions) exit 1 ;;
    esac
    shift
  ...
  read -r response || exit 1
  case "$response" in *'"behavior":"deny"'*) ;; *) exit 1 ;; esac
  emit '{"type":"stream_event","event":{"type":"content_block_delta","delta":{"type":"text_delta","text":"Login fixed; next run tests."}}}'
  emit '{"type":"result","subtype":"success","result":"Login fixed; next run tests.","usage":{"input_tokens":1,"output_tokens":1},"session_id":"sess-title"}'
  ;;
