...
- **Live Voice é engine-owned e sobrevive à navegação da UI.** Em `Working`/`AwaitingInput`, probe/start exigem a capability OMP de contexto operacional; a engine observa o Chat de origem, coalesce snapshots display-safe e nunca envia reasoning, payload/result bruto de tool ou pergunta estruturada. Delegação vocal confirmada é um comando durável `Steer`: mailbox no run vivo, um único novo turno se ele assentar antes da aplicação. O observer continua entregando progress/final ao child sem o Chat selecionado; em `Idle`, handle OMP estacionado permanece quente e elegível com Live básico.
