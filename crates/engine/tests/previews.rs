...
#[tokio::test]
async fn preview_catalog_is_available_on_the_viewing_engine() {
    ...
    core.workspace
        .create_chat(
            ...
            Some("peer"),
            None,
            Some("/peer/project".into()),
        )
        .unwrap();
    let RpcReply::Stream(mut stream) = core
        .rpc_service()
        .handle(
            "WatchPreviews",
            serde_json::json!({"chatId":"preview", "targetDeviceId":"peer"}),
        )
        .await
        .expect("preview watch remains local to the viewer")
    else {
    ...
    assert_eq!(snapshot["remote"], true);
    assert_eq!(snapshot["projectName"], "project");
    drop(stream);
    core.shutdown().await;
}
