...
async fn quiet_acp_prompt_stays_working_until_response() {
    ...
    let tools_completed = tokio::time::timeout(Duration::from_secs(10), async {
        loop {
            if doc.read_entries().unwrap_or_default().iter().any(|entry| {
                entry
                    .parts
                    .iter()
    ...
    .await;
    assert!(
        tools_completed.is_ok(),
        "tools did not complete: entries={:?}, commands={:?}, status={:?}",
        doc.read_entries(),
        doc.read_commands(),
        core.sessions.session_status(chat)
    );
