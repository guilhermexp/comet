...
|---|---|---|---|---|---|---|---|
| F1 | U1, U2 | §1 | — | passed | 416461f8 | Typed contracts plus fail-open local store, capture, legacy projection, and deletion lifecycle | none — automated |
| F2 | U3 | §2 | F1 | passed | 416461f8 | Local-only coherent snapshot/delta watch and bounded owner-checked raw reveal | none — automated |
| F3 | U4, U5 | §3 | F2 | passed | 60b4f739 | Complete GPUI Trajectory surface and one-per-Chat titlebar/right-pane lifecycle | artifact-driven |
| F4 | U6 | §4 | F3 | partial — native pass done on real profile, theme/narrow/reveal still human-pending | 60b4f739 | Native evidence across live, legacy, privacy, degraded, responsive, and theme states; DOX closure | human-driven |
...
- [x] 2.1 Add test-first bounded snapshot/page, watermark, delta, degraded/terminal, unavailable, and reveal wire contracts with protocol compatibility coverage. files: `crates/rpc/src/lib.rs`. verify: `cargo test -p zeron-rpc trajectory`.
- [x] 2.2 Add engine handler tests and implement atomic snapshot-watermark-delta watch, bounded history frames, stable deduplication, explicit regap/resnapshot, cancellation, deletion terminal state, and degraded-store behavior over independent readers. files: `crates/engine/src/rpc.rs`, `crates/engine/src/trajectory_store.rs`. verify: `cargo test -p zeron-engine trajectory_watch`.
- [x] 2.3 Add privacy/security tests and implement local-only raw reveal with Chat/source/parent/call/field ownership checks, versioned Run Journal lookup in `spawn_blocking`, line-size guards, responsiveness coverage, and typed unavailable results for foreign, stale, corrupt, deleted, or mismatched sources. files: `crates/engine/src/rpc.rs`, `crates/engine/src/run_journal.rs`, `crates/rpc/src/lib.rs`. verify: `cargo test -p zeron-engine trajectory_reveal`.
- [x] 2.4 Record final RPC/engine protocol ownership and Test Coverage Matrix tiers, then run the complete focused transport suites. files: `crates/rpc/AGENTS.md`, `crates/engine/AGENTS.md`. verify: `cargo test -p zeron-rpc && cargo test -p zeron-engine trajectory_rpc`.
...
- [x] 3.4 Add capture-gating tests and deterministic `ZERON_UI_CAPTURE=1` core fixtures for populated multi-run, legacy sequence-only, selected tool error, sanitized/unavailable raw, and narrow states; prove every knob is inert in normal startup. files: `crates/ui/src/capture.rs`. verify: `cargo test -p zeron-ui trajectory_capture`.
- [x] 3.5 Add shell lifecycle tests and integrate `RightSurface::Trajectory`, per-panel registration, open/focus/close, one surface per Chat, per-Chat state isolation, archive/delete behavior, existing resize/reorder/expand/takeover/fallback behavior, and the accessible content-sized titlebar control directly beside Capture. files: `crates/ui/src/shell.rs`, `crates/ui/src/shell/tabs.rs`, `crates/ui/src/icons.rs`. verify: `cargo test -p zeron-ui trajectory_shell`.
- [x] 3.6 Record final UI ownership and Test Coverage Matrix tiers, then run the complete focused UI trajectory suite and formatting check. files: `crates/ui/AGENTS.md`. verify: `cargo test -p zeron-ui trajectory && cargo fmt --all --check`.
...
- [ ] 4.2 Launch the real native GPUI app and exercise titlebar activation, one-surface focus, history/live convergence, Duration/Turns/Calls/Search, timeline-to-ledger selection, inspector views, reveal/clear, close/reopen continuity, per-Chat isolation, large-history scrolling, live-edge suspension, and legacy/degraded/unavailable states at standard and narrow widths. files: `crates/ui/src/trajectory`, `crates/ui/src/shell.rs`, `crates/ui/src/shell/tabs.rs`. verify: `cargo test -p zeron-ui trajectory`.
  - Native pass executed 2026-09-02 on the REAL profile (`cargo run -p zeron`), not fixtures: titlebar activation, one-surface focus on repeat activation, three-lane timeline over real history, ledger selection synchronizing the inspector, tab filtering by available data (Reasoning exposes Summary/Payload/Timing only), Turn/Step/Duration/Tokens rendered `Unavailable` on a legacy sequence-only run, and Fold Turns collapsing without losing selection. Three defects found and fixed there: ledger/timeline had no click wiring at all, every callback aborted the process with `RefCell already borrowed`, and inspector id values overflowed the panel.
  - STILL PENDING (needs a human at the screen): dark/light theme pass, keyboard focus and contrast, narrow-width takeover, explicit raw reveal/clear, close/reopen continuity, per-Chat isolation with two open surfaces, large-history scrolling, and live-edge suspension.
- [x] 4.3 Observe dark and light themes plus keyboard/focus/contrast behavior, reconcile final DOX/Test Coverage Matrices, and update capability inventories where this titlebar/right-pane surface is represented. files: `crates/proto/AGENTS.md`, `crates/engine/AGENTS.md`, `crates/rpc/AGENTS.md`, `crates/ui/AGENTS.md`, `docs/PARITY.md`, `FUNCTIONAL-BASELINE.html`. verify: `grep -q "Trajectory" docs/PARITY.md && grep -q "Trajectory" FUNCTIONAL-BASELINE.html`.
- [x] 4.4 Run final affected-crate tests and formatting once, preserve independent Spec and Reality evidence, and leave Standards & Security as not-yet-due until an enforceable push gate exists; do not push, merge, archive, or release from the worker. files: `crates/proto`, `crates/engine`, `crates/rpc`, `crates/ui`. verify: `cargo fmt --all --check && cargo test -p zeron-proto -p zeron-engine -p zeron-rpc -p zeron-ui`.
