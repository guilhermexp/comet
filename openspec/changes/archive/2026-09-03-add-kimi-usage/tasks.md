...
|---|---|---|---|---|---|---|---|
| F1 | K1-K5 | §1-§5 | — | complete | — | Kimi managed quota end-to-end | human-driven |
...
- [x] Build and visually confirm Kimi weekly and 5-hour rows in the real GPUI app using the authenticated local subscription. files: `apps/zeron`, `crates/ui`. verify: `cargo build -p zeron` plus headed-app smoke.
- [x] Validate and archive the OpenSpec change after review passes. files: `openspec/changes/add-kimi-usage`. verify: `openspec validate add-kimi-usage --strict --no-interactive`.
