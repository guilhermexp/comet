...
**must_haves:** grep maps to ToolCall::Search with pattern and optional path; glob maps to ToolCall::Glob extracting pattern from path.
...
**must_haves:** worker e subagente nunca contribuem; ancestral registrado nunca casa; own checkout nunca aparece; only assistant tool calls from the chat itself contribute; first contact order is preserved.
...
**must_haves:** section appears only in Orchestrator mode when count > 0; row clicks call reveal_project; viewport is bounded with scrolling; collapse state persists.
...
**must_haves:** AGENTS.md local contracts and Test Coverage Matrix updated; all local gates pass cleanly.

- [x] C5 Update `crates/ui/AGENTS.md` and `crates/harness/AGENTS.md` Test Coverage Matrix and local rules; run all formatting and test gates. files: `crates/ui/AGENTS.md, crates/harness/AGENTS.md`. verify: `cargo test -p zeron-ui worked_projects && cargo test -p zeron-harness omp && cargo fmt --all`.
