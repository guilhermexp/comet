...
- **Test:** none — no retained automated argv test; discovery is proven by the catalog smoke below
...
- **Test:** manual — new `OmpProcess` Session (not an already-running Chat child), then `get_available_commands` or `discover_commands_with_launch`. Throwaway smoke is not kept in tree.
...
- **Test:** harness integration (`every_omp_launch_scopes_skills_to_the_project`)
