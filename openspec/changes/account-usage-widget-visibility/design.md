...
6. **Keep `NotSignedIn` in the render enum** but stop producing it. Empty membership is zero rows plus empty-state copy.
...
- Kimi/Antigravity specs that promised a not-signed-in placeholder are updated: omission is the honest empty state.
