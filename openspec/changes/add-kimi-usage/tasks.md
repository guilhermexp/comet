...
- [ ] K3 Resolve `${KIMI_SHARE_DIR:-~/.kimi}/credentials/kimi-code.json`, reject unsafe sources, parse redacted OAuth state, and cover missing/malformed/expired/valid files. files: `crates/engine/src/kimi_usage.rs`, `crates/engine/src/lib.rs`. verify: `cargo test -p zeron-engine kimi`.
- [ ] K4 Implement near-expiry refresh through the official OAuth endpoint with cross-process locking, post-lock re-read, atomic `0600` replacement, and no token logging. files: `crates/engine/src/kimi_usage.rs`. verify: `cargo test -p zeron-engine kimi`.
...
- [ ] K5 Fetch `/usages`, parse string/numeric counters and reset windows, and feed results into the existing device-local snapshot refresh. files: `crates/engine/src/kimi_usage.rs`, `crates/engine/src/agent_accounts.rs`, `crates/proto/src/entities.rs`. verify: `cargo test -p zeron-engine kimi`.
...
- [ ] Run focused proto, engine Kimi OAuth/usage, and UI Usage tests. files: `crates/proto`, `crates/engine`, `crates/ui`. verify: `cargo test -p zeron-proto && cargo test -p zeron-engine kimi && cargo test -p zeron-ui usage`.
- [ ] Run workspace checks. files: `Cargo.toml`, `Cargo.lock`. verify: `cargo check --workspace && cargo fmt --all --check && git diff --check`.
