...
- [x] C3 Provar que `SetChatCwd` e `SwitchRef` já bastam — nenhuma mudança em `crates/engine`. Se um teste de engine for necessário para provar o retarget, escrevê-lo; não presumir. files: `crates/engine/tests/` (somente se necessário). verify: `cargo test -p zeron-engine` (escopado ao teste tocado).
