...
The transcript SHALL show the individual cards of every tool group projected
inside an expanded `TurnSteps` disclosure by default.
...
- **WHEN** completed Thought and tool activity is projected into `TurnSteps`
...
The transcript SHALL keep explicit group fold state authoritative over the
default and SHALL preserve existing top-level and streaming behavior.
