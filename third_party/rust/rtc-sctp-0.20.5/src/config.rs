...
// Comet: leave room for DTLS + UDP/IPv6 within a 1280-byte VPN path.
pub(crate) const INITIAL_MTU: u32 = 1180;
...
mod comet_mtu_tests {
    ...
    #[test]
    fn default_packets_fit_ipv6_minimum_mtu_with_dtls() {
        // IPv6 header + UDP + a DTLS 1.2 AES-GCM record (header, nonce, tag).
        let overhead = 40 + 8 + 13 + 8 + 16;
        let sctp_packet = EndpointConfig::default().get_max_payload_size()
            + COMMON_HEADER_SIZE + DATA_CHUNK_HEADER_SIZE;
        assert!(sctp_packet + overhead <= 1280,
            "SCTP packet plus DTLS/UDP/IP exceeds a 1280-byte VPN path");
    }
}
