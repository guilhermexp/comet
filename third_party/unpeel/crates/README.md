...
| `unpeel-tui` | The `unpeel` binary: terminal UI over the same hosted sessions, and the headless host on servers |
| `unpeel-native-bridge` | Panic-contained C ABI so the Swift Mac app can call unpeel-core directly |
| `unpeel-ui` | Portable Ratatui-shaped UI SDK for Unpeel Apps |
...
integration suite: `crates/unpeel-tui/tests/run.sh`.
