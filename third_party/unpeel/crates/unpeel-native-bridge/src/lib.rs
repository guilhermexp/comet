...
use unpeel_core::remote_session_backend::{
    ...
    RemoteEffectFailureKind, RemoteOutputPage, RemoteOutputPollOptions,
    RemoteProjectOrganizationPatch, RemoteSessionBackend, RemoteSessionBackendError,
    RemoteSessionCreateRequest, RemoteSessionSummary, RemoteTextSubmitMode,
    RemoteTranscriptMarkdown,
};
...
pub unsafe extern "C" fn unpeel_native_bridge_remote_output_poll(
    ...
    *out_bytes_length = 0;
    ...
    let outcome = catch_unwind(AssertUnwindSafe(|| {
        ...
        poll_remote_output(handle, session_id, limit, wait_ms)
    }));
    match outcome {
        Ok(Ok(output)) => {
...
/// colorID?, dateSorted?; absent fields are left unchanged). `sortOrder`
