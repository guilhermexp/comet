...
//! - `unpeel-host __mcp__` (Unpeel Sessions MCP over stdio; this is the
...
use unpeel_core::{
    browser_mcp, computer_mcp, mcp_gate, mcp_host, remote_attach, remote_server, remote_stdio,
    session_host, terminal_viewport, transcripts,
};
...
fn main() {
    ...
    // `unpeel-host __resume__ <session-id> [--fresh|--fork]` — print the
    ...
    if args.first().map(String::as_str) == Some("__resume__") {
        ...
        if session_id.is_empty() {
            eprintln!("usage: unpeel-host __resume__ <session-id> [--fresh|--fork]");
            std::process::exit(2);
        }
        let mode = if args.iter().any(|a| a == "--fork") {
            unpeel_core::session_ops::RelaunchMode::Fork
        } else {
            unpeel_core::session_ops::RelaunchMode::Restart {
