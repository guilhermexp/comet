...
fn archive_session_unlocked(session_id: &str) -> Result<(), String> {
    ...
    // sweep — the row floats to the top of the stopped group and lingers.
...
pub(crate) const APPENDED_CONTEXT_MARKER: &str = "appended-context.json";

/// The exact on-disk marker used to derive one relaunch command. `raw` is
/// intentionally retained: context text alone is not a safe compare token.
/// A user can publish the same text again while Resume Agent is preparing its
/// relaunch, and that later write is a distinct next-generation intent.
#[derive(Clone, Debug)]
pub(crate) struct AppendedContextSnapshot {
    context: String,
    raw: Vec<u8>,
}

/// A marker moved aside for the few microseconds between the final snapshot
/// comparison and submission of the relaunch bytes. The cross-process lock is
/// held for this object's whole lifetime, so every compliant Rust/Swift writer
/// publishes only after commit or rollback has made the old snapshot final.
pub(crate) struct StagedAppendedContext {
    original: PathBuf,
    staged: Option<PathBuf>,
    resolved: bool,
    #[allow(dead_code)]
    lock: crate::app_state::FileLock,
}

impl StagedAppendedContext {
    ...
    fn rollback_unresolved(&self) {
        ...
        // The lock prevents a cooperating writer from changing `original`
        // until this object is dropped. Keep the existence check as a
        // fail-safe for an older process that does not yet share the lock.
        if !self.original.exists() {
            let _ = std::fs::rename(staged, &self.original);
        } else {
            let _ = std::fs::remove_file(staged);
        }
    }
...
impl Drop for StagedAppendedContext {
    fn drop(&mut self) {
        if !self.resolved {
            self.rollback_unresolved();
        }
...
fn appended_context_lock_target_at(lock_dir: &std::path::Path, session_id: &str) -> PathBuf {
    lifecycle_lock_target_at(lock_dir, session_id)
}

fn lock_appended_context_at(
    lock_dir: &std::path::Path,
    session_id: &str,
) -> Result<crate::app_state::FileLock, String> {
    std::fs::create_dir_all(lock_dir).map_err(|e| e.to_string())?;
    crate::app_state::lock_exclusive(&appended_context_lock_target_at(lock_dir, session_id))
}

fn appended_context_lock_dir() -> Result<PathBuf, String> {
    Ok(app_paths::ensure_unpeel_home()
        .map_err(|e| e.to_string())?
        .join("session-appended-context-locks"))
}

fn appended_context_snapshot_at(
    marker: &std::path::Path,
    lock_dir: &std::path::Path,
    session_id: &str,
) -> Result<Option<AppendedContextSnapshot>, String> {
    let _lock = lock_appended_context_at(lock_dir, session_id)?;
    let raw = match std::fs::read(marker) {
        Ok(raw) => raw,
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => return Ok(None),
        Err(error) => return Err(format!("read {}: {error}", marker.display())),
    };
    let Some(context) = serde_json::from_slice::<serde_json::Value>(&raw)
        .ok()
        .and_then(|value| value.get("context")?.as_str().map(str::to_owned))
        .filter(|context| !context.is_empty())
    else {
        // Preserve unknown/corrupt bytes. They are not safe to inject, and an
        // exact future writer can still replace or clear them.
        return Ok(None);
    };
    Ok(Some(AppendedContextSnapshot { context, raw }))
}

fn stage_appended_context_snapshot_at(
    marker: &std::path::Path,
    lock_dir: &std::path::Path,
    session_id: &str,
    snapshot: &AppendedContextSnapshot,
    generation: u64,
) -> Result<Option<StagedAppendedContext>, String> {
    let lock = lock_appended_context_at(lock_dir, session_id)?;
    ...
    if current != snapshot.raw {
        // A writer published after command derivation. The relaunch still
        // consumes its exact old snapshot, while this marker remains pending
        // for the following generation (even when the text is identical).
        return Ok(None);
    }
    let staged = marker.with_file_name(format!(
        ".{APPENDED_CONTEXT_MARKER}.restart-{}-{generation}",
        std::process::id()
    ));
    let _ = std::fs::remove_file(&staged);
    std::fs::rename(marker, &staged).map_err(|e| {
        format!(
            "Failed to stage {} for agent restart: {e}",
            marker.display()
        )
    })?;
    // Re-check after the rename as a compatibility guard for an older writer
    // that does not yet honor this lock and could have atomically replaced the
    // canonical path between our read and rename. Never consume those bytes.
    let staged_raw = match std::fs::read(&staged) {
        Ok(raw) => raw,
        Err(error) => {
            if !marker.exists() {
                let _ = std::fs::rename(&staged, marker);
            }
            return Err(format!("read staged {}: {error}", staged.display()));
        }
    };
    if staged_raw != snapshot.raw {
        if !marker.exists() {
            std::fs::rename(&staged, marker).map_err(|e| {
                format!(
                    "Failed to restore concurrently published {}: {e}",
                    marker.display()
                )
            })?;
        } else {
    ...
    Ok(Some(StagedAppendedContext {
        original: marker.to_path_buf(),
        staged: Some(staged),
        resolved: false,
        lock,
    }))
...
/// Lock the exact appended-context state used to derive a replacement
/// Session's command. Unlike the live in-place path, replacement deletes the
/// old Session directory; a writer that lands after command derivation would
/// otherwise report success and then have its newer marker silently removed.
/// A mismatch fails before teardown, while an exact missing/present snapshot
/// remains locked until the caller commits or aborts the replacement.
fn lock_replacement_appended_context_at(
    marker: &std::path::Path,
    lock_dir: &std::path::Path,
    session_id: &str,
    snapshot: Option<&AppendedContextSnapshot>,
    generation: u64,
) -> Result<StagedAppendedContext, String> {
    let lock = lock_appended_context_at(lock_dir, session_id)?;
    let current = match std::fs::read(marker) {
        Ok(raw) => Some(raw),
    ...
    let expected = snapshot.map(|snapshot| snapshot.raw.as_slice());
    if current.as_deref() != expected {
        return Err(
            "Pending appended context changed while Resume was being prepared; retry Resume".into(),
        );
    }

    let staged = if current.is_some() {
        let staged = marker.with_file_name(format!(
            ".{APPENDED_CONTEXT_MARKER}.replace-{}-{generation}",
            std::process::id()
        ));
        let _ = std::fs::remove_file(&staged);
        std::fs::rename(marker, &staged).map_err(|error| {
            format!(
                "Failed to stage {} for Session Resume: {error}",
                marker.display()
            )
        })?;
        // Compatibility guard for a pre-lock writer racing the read/rename.
        if std::fs::read(&staged).ok().as_deref() != expected {
            if !marker.exists() {
                let _ = std::fs::rename(&staged, marker);
            } else {
                let _ = std::fs::remove_file(&staged);
            ...
            return Err(
                "Pending appended context changed while Resume was being prepared; retry Resume"
                    .into(),
            );
        }
        Some(staged)
...
fn lock_replacement_appended_context(
    session_id: &str,
    snapshot: Option<&AppendedContextSnapshot>,
    generation: u64,
) -> Result<StagedAppendedContext, String> {
    lock_replacement_appended_context_at(
        &session_dir(session_id).join(APPENDED_CONTEXT_MARKER),
        &appended_context_lock_dir()?,
        session_id,
        snapshot,
        generation,
    )
...
pub(crate) fn stage_appended_context_snapshot(
    session_id: &str,
    snapshot: &AppendedContextSnapshot,
    generation: u64,
) -> Result<Option<StagedAppendedContext>, String> {
    stage_appended_context_snapshot_at(
        &session_dir(session_id).join(APPENDED_CONTEXT_MARKER),
        &appended_context_lock_dir()?,
        session_id,
        snapshot,
...
/// Pending appended system context: extra system-prompt text applied on the
/// session's NEXT restart, then consumed. A user action, so it announces —
/// the other UI surfaces its restart recommendation from this.
pub fn set_appended_context(session_id: &str, context: Option<&str>) -> Result<(), String> {
    let dir = session_dir(session_id);
    if !dir.exists() {
        return Err(format!("no session dir for {session_id}"));
    }
    let _lock = lock_appended_context_at(&appended_context_lock_dir()?, session_id)?;
    let trimmed = context.map(str::trim).filter(|s| !s.is_empty());
    match trimmed {
        None => {
            let _ = std::fs::remove_file(dir.join(APPENDED_CONTEXT_MARKER));
        }
        Some(text) => {
            let body = serde_json::json!({
                "context": text,
                "updated_at": now_ms(),
                // Distinguish a deliberate re-publish of identical text from
                // the snapshot already being consumed by Resume Agent.
                "revision": uuid::Uuid::new_v4().to_string(),
            });
            let tmp = dir.join(".appended-context.json.tmp");
            std::fs::write(&tmp, serde_json::to_vec(&body).map_err(|e| e.to_string())?)
                .map_err(|e| e.to_string())?;
            std::fs::rename(&tmp, dir.join(APPENDED_CONTEXT_MARKER)).map_err(|e| e.to_string())?;
        }
    }
    crate::state_bus::announce(
        crate::state_bus::Change::SessionMarkers,
...
pub fn appended_context_marker(session_id: &str) -> Option<String> {
    appended_context_snapshot_at(
        &session_dir(session_id).join(APPENDED_CONTEXT_MARKER),
        &appended_context_lock_dir().ok()?,
        session_id,
    )
    .ok()?
...
/// archived, but must neither float to the top of the stopped group nor
...
fn rewrite_shared_session_references(
    ...
    let app_state_result = crate::app_state::edit(|state| {
        Ok(rewrite_app_state_session_references(
            state,
            old_id,
            replacement_id,
        ))
...
fn remove_session_unlocked(session_id: &str) -> Result<(), String> {
    let managed_storage = load_manifest(session_id)
        .as_ref()
        .and_then(managed_storage_for_manifest);
...
pub(crate) fn browser_mcp_enabled_from_app_state(state: &serde_json::Value) -> bool {
    ...
    access != BrowserAccess::Off
}

fn browser_mcp_enabled_for_launch() -> bool {
    match crate::app_state::load() {
        Ok(state) => browser_mcp_enabled_from_app_state(&state),
        Err(error) => {
            // A damaged settings file must not take down Session creation,
            // but it also must not silently grant browser access.
            log::warn!("Could not read Browser MCP launch policy; disabling it: {error}");
            false
        }
    }
}
...
pub fn spawn_session(
    ...
    let browser_mcp_enabled = browser_mcp_enabled_for_launch();
...
pub enum RelaunchMode {
    ...
    /// Branch it: a new provider conversation seeded from the old one.
    Fork,
}

/// The command a relaunch of this session should run — resume tiers, the
/// pending appended context, fork rewriting — derived entirely from shared
/// on-disk state. THE one implementation: the TUI restart, the CLI, and the
/// app (via `unpeel-host __resume__`) all call this, which is what keeps a
/// restart identical no matter which UI performed it.
pub(crate) fn relaunch_command_and_pending_context(
    session_id: &str,
    mode: RelaunchMode,
) -> Result<(String, Option<AppendedContextSnapshot>), String> {
    ...
    let mut relaunch = match mode {
        RelaunchMode::Fork => crate::resume::forked(&old.session.command, provider_id.as_deref())
            .ok_or_else(|| {
            format!(
                "{} has no native fork primitive",
                crate::integrations::command_head(&old.session.command)
            )
        })?,
    ...
    // Pending appended system context is a restart-only launch flag: apply
    // it to the relaunch and let the marker die with the old session dir —
    // the new session's command carries the context permanently.
    let pending_context = appended_context_snapshot_at(
        &session_dir(session_id).join(APPENDED_CONTEXT_MARKER),
        &appended_context_lock_dir()?,
        session_id,
    )?;
    if let Some(snapshot) = pending_context.as_ref() {
        relaunch = crate::provider_context::appended_command(&relaunch, &snapshot.context);
    }
    Ok((relaunch, pending_context))
}

pub fn relaunch_command(session_id: &str, mode: RelaunchMode) -> Result<String, String> {
    relaunch_command_and_pending_context(session_id, mode).map(|(command, _)| command)
}
...
fn restart_session_unlocked(
    ...
    let old = manifest(session_id)?;
    let (relaunch, pending_context) = relaunch_command_and_pending_context(
        session_id,
        RelaunchMode::Restart { force_fresh: false },
    )?;
    ...
    let context_transaction = lock_replacement_appended_context(
        session_id,
        pending_context.as_ref(),
        old.runtime_launch_generation.saturating_add(1),
    )?;
    teardown_session_files(session_id)?;
...
mod tests {
    use super::{
        appended_context_lock_target_at, appended_context_snapshot_at,
        browser_mcp_enabled_from_app_state, initial_text_payload, lifecycle_lock_target_at,
        lock_appended_context_at, lock_replacement_appended_context_at,
        merge_project_sibling_order, rewrite_app_state_session_references,
        rewrite_session_order_references_at, rewrite_session_order_value,
        stage_appended_context_snapshot_at, validate_session_order_references_at,
        InitialTextSubmitMode,
    };

    #[test]
    fn appended_context_lock_path_matches_the_cross_process_contract() {
        let root = std::path::Path::new("/tmp/unpeel-test");
        assert_eq!(
            appended_context_lock_target_at(root, "session"),
            root.join("3f3af1ecebbd1410ab417ec0d27bbfcb5d340e177ae159b59fc8626c2dfd9175")
        );
    }

    #[test]
    fn restart_stages_only_the_exact_appended_context_snapshot() {
        let temp = tempfile::tempdir().unwrap();
        let marker = temp.path().join("session/appended-context.json");
        let lock_dir = temp.path().join("session-appended-context-locks");
        std::fs::create_dir_all(marker.parent().unwrap()).unwrap();
        let first = br#"{"context":"same text","updated_at":1,"revision":"first"}"#;
        let second = br#"{"context":"same text","updated_at":2,"revision":"second"}"#;
        std::fs::write(&marker, first).unwrap();
        let snapshot = appended_context_snapshot_at(&marker, &lock_dir, "session")
            .unwrap()
            .unwrap();

        // A new publication can intentionally contain the same text. Its
        // exact bytes/revision still make it the next generation's intent.
        std::fs::write(&marker, second).unwrap();
        assert!(
            stage_appended_context_snapshot_at(&marker, &lock_dir, "session", &snapshot, 2)
                .unwrap()
                .is_none()
        );
        assert_eq!(std::fs::read(&marker).unwrap(), second);
    }

    #[test]
    fn restart_rollback_restores_before_next_context_publication() {
        ...
        let lock_dir = temp.path().join("session-appended-context-locks");
        std::fs::create_dir_all(marker.parent().unwrap()).unwrap();
        let first = br#"{"context":"old","updated_at":1,"revision":"first"}"#;
        let next = br#"{"context":"next","updated_at":2,"revision":"second"}"#.to_vec();
        std::fs::write(&marker, first).unwrap();
        let snapshot = appended_context_snapshot_at(&marker, &lock_dir, "session")
            .unwrap()
            .unwrap();
        ...
        // A separate file description cannot take the writer lock while the
        // staged transaction is unresolved. This is deterministic and checks
        // the same flock contract used across separate Rust/Swift processes.
        let lock_file = std::fs::OpenOptions::new()
            .write(true)
            .open(appended_context_lock_target_at(&lock_dir, "session").with_extension("lock"))
            .unwrap();
        assert_ne!(
        ...
        // A failed PTY submission restores the consumed snapshot before the
        // next writer can publish. The writer then wins as the newer intent.
        staged.rollback();
        assert_eq!(std::fs::read(&marker).unwrap(), first);
        let _writer_lock = lock_appended_context_at(&lock_dir, "session").unwrap();
        std::fs::write(&marker, next).unwrap();
        assert_eq!(
            std::fs::read_to_string(&marker).unwrap(),
            r#"{"context":"next","updated_at":2,"revision":"second"}"#
        );
    }

    #[test]
    fn replacement_resume_rejects_a_newer_context_before_teardown() {
        let temp = tempfile::tempdir().unwrap();
        let marker = temp.path().join("session/appended-context.json");
        let lock_dir = temp.path().join("session-appended-context-locks");
        std::fs::create_dir_all(marker.parent().unwrap()).unwrap();
        let first = br#"{"context":"old","updated_at":1,"revision":"first"}"#;
        let second = br#"{"context":"new","updated_at":2,"revision":"second"}"#;
        std::fs::write(&marker, first).unwrap();
        let snapshot = appended_context_snapshot_at(&marker, &lock_dir, "session")
            .unwrap()
            .unwrap();
        ...
        let error = match lock_replacement_appended_context_at(
            &marker,
            &lock_dir,
            "session",
            Some(&snapshot),
            ...
            Ok(_) => panic!("newer context must abort replacement before teardown"),
            Err(error) => error,
        };
        assert!(error.contains("changed while Resume was being prepared"));
        assert_eq!(std::fs::read(&marker).unwrap(), second);
    }
