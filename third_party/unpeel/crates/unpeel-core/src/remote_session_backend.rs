...
impl RemoteSessionBackend {
    ...
    pub fn poll_output(
        ...
        self.inner.poll_output(session_id, options)
    }
...
impl BackendInner {
    ...
    fn perform_effect(
        ...
        let exchange =
            self.perform_effect_exchange(effect_turn, operation, capability, path, body)?;
    ...
    fn perform_effect_exchange(
        ...
        let call = HostCall::new("POST", path, RequestSemantics::Effect)
...
mod tests {
    ...
    fn lifecycle_and_organization_effects_use_v1_wire_contracts() {
        ...
        assert_eq!(backend.archive_session("s1").unwrap().request_id(), 4);
        assert_eq!(backend.restore_session("s1").unwrap().request_id(), 5);
        assert_eq!(backend.stop_session("s1").unwrap().request_id(), 6);
        assert_eq!(backend.remove_session("s1").unwrap().request_id(), 7);
        assert_eq!(backend.restart_session("s1").unwrap().request_id(), 8);
        assert_eq!(backend.restart_agent("s1").unwrap().request_id(), 9);
        assert_eq!(backend.resume_agent("s1").unwrap().request_id(), 10);
        assert_eq!(
            backend
