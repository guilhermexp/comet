...
const MAX_RESPONSE_BODY_BYTES: usize = relay_crypto::MAX_PLAINTEXT_BYTES;
...
impl DirectHostConnection {
    ...
    fn perform_http(
        &self,
        generation: &Arc<HttpGeneration>,
        request: &crate::relay_crypto::TunnelRequest,
        request_head: &[u8],
        timeout: Duration,
...
impl HostConnection for DirectHostConnection {
    ...
    fn request(
        ...
        let encoded = relay_crypto::encode_tunnel_request(&request);
        if !relay_crypto::plaintext_frame_fits(encoded.len()) {
        ...
        if http_bytes > relay_crypto::MAX_PLAINTEXT_BYTES {
            return Err(HostConnectionError::RequestTooLarge {
                request_id,
