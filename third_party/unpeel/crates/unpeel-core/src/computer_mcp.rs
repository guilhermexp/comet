...
//! Mac itself — window screenshots + accessibility-tree maps, background
//! clicking/typing, app/window control — through the embedded **cua-driver**
//! engine (github.com/trycua/cua, `libs/cua-driver`).
//!
//! Design doc: `docs/feature/computer-mcp.md` (engine switched from Peekaboo
...
//!
//! - The **native app** owns a long-lived daemon: it spawns
...
fn load_security() -> ComputerSecurity {
    ...
    let default_access = match value.get("computer_default_access").and_then(Value::as_str) {
...
/// Engine resolution, same order as the browser engine: env override →
/// bundled next to unpeel-host → managed dir → PATH (dev).
fn resolve_engine_binary() -> Result<PathBuf, String> {
    if let Ok(value) = std::env::var("UNPEEL_CUA_DRIVER_BIN") {
    ...
    if let Some(found) = crate::setup::find_command_path("cua-driver", &crate::setup::search_dirs())
    {
        return Ok(PathBuf::from(found));
    }
    Err(
        "The computer-use engine (cua-driver) is not available. Expected it bundled with \
Unpeel, in ~/.unpeel/computer/bin/, or on PATH."
            .into(),
    )
}
...
/// macOS permission status so an agent can discover why tools refuse.
