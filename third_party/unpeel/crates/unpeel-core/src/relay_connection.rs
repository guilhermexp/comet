...
impl HostConnection for RelayHostConnection {
    ...
    fn request(
        ...
        let encoded_request = relay_crypto::encode_tunnel_request(&request);
        if !relay_crypto::plaintext_frame_fits(encoded_request.len()) {
        ...
        let response = match relay_crypto::parse_tunnel_response(&transport.encoded_response) {
...
mod tests {
    ...
    struct ScriptedExecutor {
        ...
        calls: Mutex<Vec<(relay_crypto::TunnelRequest, Option<u64>, Duration)>>,
    ...
    impl RelayRequestExecutor for ScriptedExecutor {
        fn request(
            ...
            let request = relay_crypto::parse_tunnel_request_strict(encoded_request).unwrap();
            self.calls
                .lock()
                .unwrap()
    ...
    fn reply(id: u64, generation: u64, status: u16, body: &[u8]) -> RelayTransportReply {
        RelayTransportReply {
            ...
            encoded_response: relay_crypto::encode_tunnel_response(id, status, body),
        }
    }
