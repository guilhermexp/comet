...
pub fn run_stdio(kind: &str) -> Result<(), String> {
    ...
    if enabled {
        return match kind {
            ...
            // Per-domain kinds delegate to the same unified server, bounded to
            // that one domain: stale per-domain entries keep working after a
            // binary update without inheriting other manifest grants.
            SESSIONS_KIND => {
