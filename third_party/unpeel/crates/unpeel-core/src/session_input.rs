...
pub fn deliver_sanitized_text(
    ...
    deliver_sanitized_text_with_timeout(session_id, sanitized, submit, None, None)
}

pub fn deliver_sanitized_text_with_task_receipt(
    session_id: &str,
    sanitized: &str,
    task_episode: u64,
) -> Result<(), String> {
    deliver_sanitized_text_with_timeout(session_id, sanitized, true, None, Some(task_episode))
}
...
fn deliver_sanitized_text_with_timeout(
    ...
    submit: bool,
    timeout: Option<Duration>,
    task_episode: Option<u64>,
) -> Result<(), String> {
    if !sanitized.is_empty() {
        write_to_session(
            session_id,
            &encode_bracketed_paste(sanitized),
            timeout,
            None,
            None,
        )?;
    ...
    if submit {
        ...
        write_to_session(
            ...
            "\r",
            timeout,
            task_episode.map(|episode| format!("comet-task-{session_id}-{episode}")),
            task_episode,
        )?;
        thread::sleep(Duration::from_millis(ENTER_FOLLOWUP_DELAY_MS));
        write_to_session(session_id, "\r", timeout, None, None)?;
    }
    Ok(())
}
...
fn request_screenshot_with_optional_timeout(
    ...
    deliver_sanitized_text_with_timeout(session_id, &sanitized, true, timeout, None)
}
