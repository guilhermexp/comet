...
pub fn append_trace_log_line(line: &str) {
    let trace_path = app_hooks_root().join("trace.log");
...
pub(crate) fn notify_hook_script_path() -> PathBuf {
    app_hooks_root().join("notify-hook.sh")
}
...
pub use crate::integrations::{
    ...
    claude::setup::{claude_mcp_config_path, claude_unpeel_mcp_config_path, install_claude_hooks},
    ...
    kimi::setup::{
        install_kimi_hooks, kimi_global_mcp_config_path, kimi_mcp_config_path,
