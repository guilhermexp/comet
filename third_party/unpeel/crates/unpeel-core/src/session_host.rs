...
const SESSION_SOCKET_ACCEPT_POLL_MS: u64 = 4;
...
const SESSION_OUTPUT_STREAM_RECENT_BYTES: usize = 1024 * 1024;
...
pub struct SessionHostLaunch {
    ...
    /// True only for sessions granted Browser Access: the unified MCP server
    /// advertises the `browser` domain to this session. Defaults to false —
    /// browser automation can reach logged-in sites, so access is opt-in per
    /// session.
    #[serde(default)]
...
fn current_host_build_id() -> Option<String> {
...
pub enum SessionHostCommand {
    Write {
        ...
        /// Optional Comet delegation episode receipt. The host persists it
        /// after the PTY write and before acknowledging this command, so a
        /// controller never reports submission success without durable
        /// recovery evidence.
        #[serde(default, skip_serializing_if = "Option::is_none")]
...
#[cfg(unix)]
fn wait_for_pty_output(fd: Option<std::os::unix::io::RawFd>, running: &AtomicBool) -> bool {
    let Some(fd) = fd else {
    ...
    loop {
        ...
        // After Kill, make one non-blocking pass at a time so already-buffered
        // termination output is still drained before a retained slave causes
        // us to leave the loop.
        let timeout_ms = if is_running {
            SESSION_OUTPUT_READ_POLL_MS
        } else {
            0
        };
        ...
        if result < 0 && std::io::Error::last_os_error().kind() != std::io::ErrorKind::Interrupted {
            // Let the read surface the concrete PTY error.
            return true;
        }
        if !is_running {
            return false;
...
fn resume_agent_in_place(
    ...
    // In particular, capture the provider identity and appended-context
    // snapshot before the replacement agent launches.
    let (relaunch_command, pending_context) =
        crate::session_ops::relaunch_command_and_pending_context(
            session_id,
    ...
    // Compare the exact marker bytes under the same cross-process lock used
    // by Rust and Swift writers. If a later intent was published while the
    // old agent was stopping, it stays canonical for the next generation.
    // When the snapshot does match, the returned guard holds that lock until
    // the PTY submission commits or rolls back.
    let context_marker_result = match pending_context.as_ref() {
        Some(snapshot) => crate::session_ops::stage_appended_context_snapshot(
            session_id,
            snapshot,
            next_generation,
        ),
    ...
    let context_marker = match context_marker_result {
        Ok(marker) => marker,
        Err(error) => {
...
pub fn socket_path(session_id: &str) -> PathBuf {
    session_dir(session_id).join("session.sock")
}
...
pub fn reap_dead_sessions(saved_session_ids: &HashSet<String>) {
    ...
    for entry in entries.flatten() {
        ...
        let should_remove = match &manifest {
            ...
            Some(m) if m.state == HostedSessionState::Running => {
                m.pid.is_none_or(|pid| !process_exists(pid))
            }
            _ => false,
        };
...
fn mark_manifest_exited(session_id: &str) {
    let _ = update_manifest_session(session_id, |manifest| {
...
fn run_host(mut launch: SessionHostLaunch) -> Result<(), String> {
    ...
    #[cfg(unix)]
    {
        ...
        // its screen-wipe). Liveness uses our own pid as a placeholder until
        // the child pid is known; the post-spawn manifest write below corrects
        // it.
        save_manifest(&HostedSessionManifest {
            ...
            pid_started_at: process_start_time_ms(std::process::id()),
            exit_code: None,
        ...
        let heartbeat_session_id = launch.session.id.clone();
        let running_for_heartbeat = Arc::clone(&running);
        let heartbeat_thread = thread::spawn(move || {
            while running_for_heartbeat.load(Ordering::Relaxed) {
                // Sleep in short ticks: shutdown joins this thread before the
                // final exit-manifest write, and a single full-interval sleep
                // held that write hostage for up to 60s — during which the
                // client shows a dead terminal instead of the exited state.
                let mut slept_ms: u64 = 0;
                while slept_ms < SESSION_HEARTBEAT_INTERVAL_MS
                    && running_for_heartbeat.load(Ordering::Relaxed)
                {
                    thread::sleep(Duration::from_millis(250));
                    slept_ms += 250;
                }
                if !running_for_heartbeat.load(Ordering::Relaxed) {
                    break;
                }
        ...
        let runtime_observer_expected_id =
            integrations::runtime_for_command(&launch.session.command)
                .map(|runtime| runtime.legacy_slug.clone());
        let runtime_observer_completion_path =
            runtime_launch_completion_path(&runtime_observer_session_id);
        let running_for_runtime_observer = Arc::clone(&running);
        ...
        let runtime_observer_thread = thread::spawn(move || {
            let (Some(session_leader_pid), Some(session_leader_started_at_ms)) =
                (pid, pid_started_at)
            else {
                return;
            };
            ...
            while running_for_runtime_observer.load(Ordering::Relaxed) {
        ...
        let viewport_for_listener = Arc::clone(&viewport);
        ...
        let input_written_for_listener = Arc::clone(&has_been_written_to);
        let listener_thread = thread::spawn(move || {
            loop {
                if !running_for_listener.load(Ordering::Relaxed) {
                    break;
                }

                match listener.accept() {
                    Ok((stream, _)) => {
                        let runtime = Arc::clone(&runtime_for_listener);
                        let viewport = Arc::clone(&viewport_for_listener);
                        let running = Arc::clone(&running_for_listener);
                        let broadcaster = Arc::clone(&broadcaster_for_listener);
                        let session_id = listener_session_id.clone();
                        let title_buffer = Arc::clone(&title_buffer);
                    ...
                    Err(error) if error.kind() == std::io::ErrorKind::WouldBlock => {
                        thread::sleep(Duration::from_millis(SESSION_SOCKET_ACCEPT_POLL_MS));
                    }
                    Err(_) => break,
                }
            }

            let _ = fs::remove_file(socket_path(&listener_session_id));
        });

        let (output_tx, output_rx) = mpsc::sync_channel::<Vec<u8>>(SESSION_OUTPUT_CHANNEL_CAPACITY);
        let output_writer_thread = thread::spawn(move || {
            run_batched_output_writer(
                retained_output,
                output_rx,
                Duration::from_millis(SESSION_OUTPUT_BATCH_FLUSH_MS),
                SESSION_OUTPUT_BATCH_MAX_BYTES,
            )
        });

        let mut buf = vec![0u8; SESSION_OUTPUT_READ_BUFFER_BYTES];
        let mut query_scanner = OutputQueryScanner::new();
        // OSC 10/11 answers report the app's terminal palette for the mode
        // this session launched in, matching what an attached surface would
        // say (dark #F3F5FB on #1A1A1F, light #111217 on #FFFFFF).
        let dark_mode = launch.dark_mode.unwrap_or(true);
        loop {
            ...
            match reader.read(&mut buf) {
                ...
                Ok(n) => {
                    ...
                    // probe set (CPR/kitty/OSC color) whenever no answering
                    // surface is connected — probe-dependent TUIs (muse exits
                    // ~4s after launch unanswered) must survive sessions
                    // started from the phone/MCP or left unviewed. See
                    // OutputQueryScanner.
                    let intercept_probes = !broadcaster.lock().unwrap().has_answering_subscriber();
                    let (chunk, host_queries) = query_scanner.scan(&buf[..n], intercept_probes);
                    if !host_queries.is_empty() {
                        let cursor = viewport.lock().unwrap().cursor_position();
                        ...
                        for query in &host_queries {
                            let _ = match query {
                                ...
                                HostAnsweredQuery::CursorPosition => guard.writer.write_all(
                                    format!("\x1b[{};{}R", cursor.0 + 1, cursor.1 + 1).as_bytes(),
                                ),
                                HostAnsweredQuery::KittyFlags => {
                                    guard.writer.write_all(b"\x1b[?0u")
                                }
                                HostAnsweredQuery::OscColor { code } => {
                                    let rgb = match (code, dark_mode) {
                    ...
                    viewport.lock().unwrap().feed(&chunk);
                    // Prioritize the active live stream before persistence so the visible
                    // terminal stays responsive even if the batched log writer is briefly
                    // backpressured.
                    broadcaster.lock().unwrap().broadcast_chunk(&chunk);
        ...
        // The child is gone; flush any held partial escape sequence so the
        // log and stream carry every byte the PTY produced.
        let pending = query_scanner.take_pending();
        if !pending.is_empty() {
            viewport.lock().unwrap().feed(&pending);
            broadcaster.lock().unwrap().broadcast_chunk(&pending);
            let _ = output_tx.send(pending);
        }
        drop(output_tx);
        broadcaster.lock().unwrap().mark_exited();
        match output_writer_thread.join() {
            Ok(Ok(())) => {}
            Ok(Err(error)) => return Err(error),
            Err(_) => return Err("Output writer thread panicked".into()),
        }

        running.store(false, Ordering::Relaxed);
        let _ = heartbeat_thread.join();
        let _ = runtime_observer_thread.join();
        ...
        // Merge the exit edge into the latest record under the cross-process
        // manifest lock. This preserves a title or provider identity that a
        // hook/native writer published while the PTY was draining. A removed
        // Session has no manifest, so update_manifest_session returns None and
        // never recreates its directory.
        let input_was_written = has_been_written_to.load(Ordering::Relaxed);
        let _ = update_manifest_session(&session_id, |manifest| {
            manifest.state = HostedSessionState::Exited;
            manifest.pid = pid;
            manifest.pid_started_at = pid_started_at;
...
mod tests {
    use super::{
        ...
        ensure_managed_storage_path, env_keys_with_prefix, extract_submitted_prompt,
        fallback_shell_exec_snippet, fish_bridge_script, fish_single_quote, load_manifest,
        ...
        process_start_time_ms, read_input_stream_frame, read_output_chunk,
        read_output_stream_frame, refresh_manifest_health_from_manifest,
        ...
        write_output_stream_frame, HostAnsweredQuery, HostedSessionManifest, HostedSessionRuntime,
        HostedSessionState, OutputBroadcaster, OutputQueryScanner, OutputStreamRead, PidIdentity,
        RetainedOutputWriter, RuntimeObservationTracker, SessionExecutionScope, SessionHostCommand,
        SessionHostLaunch, SessionOutputChunk, ShellFamily, HERDR_ENV_PREFIX,
        LEAKED_LAUNCHER_ENV_KEYS, PRIMARY_DEVICE_ATTRIBUTES_RESPONSE, SESSION_RUNTIME_CLEAR_MISSES,
    };
    #[cfg(unix)]
    use super::{
        configure_session_client, persist_task_submission_receipt_at,
        prepare_runtime_launch_completion_marker, stage_restart_marker, SESSION_INPUT_STREAM_ACK,
    };
    ...
    #[cfg(unix)]
    #[test]
    fn task_submission_receipt_is_persisted_before_host_acknowledgement() {
        let directory = tempfile::tempdir().unwrap();
        persist_task_submission_receipt_at(directory.path(), 7).unwrap();
        assert_eq!(
            std::fs::read_to_string(directory.path().join("comet-task-submitted")).unwrap(),
            "7\n"
        );
    }
    ...
    fn startup_shell_script_survives_nonzero_provider_exit() {
        ...
        assert!(script.contains("; { false; }; __unpeel_startup_status=$?; set +e; "));
...
"#,
    ...
    fn apply_manifest_auto_title_marks_blank_terminals_custom() {
        ...
        let manifest = manifest_for_auto_title(&session_id, "", "Terminal");
        save_manifest(&manifest).unwrap();
...
#[cfg(unix)]
fn persist_task_submission_receipt(session_id: &str, episode: u64) -> Result<(), String> {
    persist_task_submission_receipt_at(&session_dir(session_id), episode)
}

#[cfg(unix)]
fn persist_task_submission_receipt_at(directory: &Path, episode: u64) -> Result<(), String> {
    let path = directory.join("comet-task-submitted");
    let temporary = directory.join(format!(".comet-task-submitted.{}.tmp", std::process::id()));
    let mut file = OpenOptions::new()
        .create(true)
        .truncate(true)
        .write(true)
    ...
    fs::rename(&temporary, &path)
        .map_err(|error| format!("Failed to publish task submission receipt: {error}"))
}
...
fn handle_client(
    ...
    // The listener is nonblocking so its accept loop can observe shutdown.
    // Darwin propagates that flag to accepted sockets, while these detached
    // client handlers all use blocking framed protocols. In particular,
    // StreamInput acknowledges the handshake before waiting for its first
    // frame; leaving O_NONBLOCK set turns that intentional wait into
    // WouldBlock and can silently lose the first queued keystroke batch.
    ...
    if let SessionHostCommand::StreamOutput {
        offset,
        answers_queries,
    } = command
    {
        // Bound how long a write can block: a client that stops reading is
        // dropped instead of pinning the forwarder thread (and its backlog)
        // forever.
        let _ = stream.set_write_timeout(Some(Duration::from_millis(
            SESSION_OUTPUT_STREAM_WRITE_TIMEOUT_MS,
        )));
        let (tx, rx) = mpsc::channel();
        let Some((subscriber_id, buffered)) =
            broadcaster
                .lock()
                .unwrap()
                .subscribe(offset, tx, answers_queries)
        else {
            // Closing without a frame is the only gap-safe response supported
            // by the v1 stream framing. The attach/remote client falls back to
            // the journal and retries from a retained cursor.
            return Ok(());
        };
        let _ = run_batched_output_stream_forwarder(
            &mut stream,
            rx,
            Duration::from_millis(SESSION_OUTPUT_STREAM_BATCH_FLUSH_MS),
            SESSION_OUTPUT_STREAM_BATCH_MAX_BYTES,
            &buffered,
        );
        broadcaster.lock().unwrap().remove_subscriber(subscriber_id);
        return Ok(());
    }

    if let SessionHostCommand::StreamInput = command {
        stream
            .write_all(&[SESSION_INPUT_STREAM_ACK])
            .and_then(|_| stream.flush())
            .map_err(|e| format!("Failed to acknowledge input stream: {e}"))?;

        while let Some(data) = read_input_stream_frame(&mut reader)? {
            if data.is_empty() {
                continue;
            }
            {
            ...
            mark_input_written(session_id, has_been_written_to);
            maybe_auto_title_from_input(session_id, &data, title_buffer, title_done);
        }
        return Ok(());
    }
    ...
    let response = match command {
        SessionHostCommand::Write {
            ...
            match validate_write_id(write_id.as_deref()) {
                ...
                Ok(write_id) => {
                    ...
                    match task_episode_receipt
                        .map(|episode| persist_task_submission_receipt(session_id, episode))
                        .transpose()
                    {
                        Ok(Some(())) | Ok(None) => SessionHostResponse {
                            ok: true,
                            error: None,
                            viewport: None,
                            activity_token: None,
        ...
        SessionHostCommand::Resize { cols, rows } => {
            // Scope the runtime guard to the (cheap) PTY resize only. The
            // viewport reflow below re-wraps up to 4MB of scrollback; doing
            // it while still holding `runtime` blocked keystroke Writes and
            // the reader thread for the whole reflow — during a window drag
            // that serialized every grid change behind the previous one's
            // reflow.
            {
        ...
        SessionHostCommand::StreamInput => unreachable!(),
        SessionHostCommand::Kill => {
            ...
            let _restart_guard = agent_restart_lock
                ...
                .unwrap_or_else(|poisoned| poisoned.into_inner());
            terminate_hosted_runtime(&mut runtime.lock().unwrap());
            // Let the output thread drain anything emitted during graceful
