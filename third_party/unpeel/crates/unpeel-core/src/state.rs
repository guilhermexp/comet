...
/// What happens when a session tries to write (send_text/send_keys) to a
/// session outside its effective sidebar group. Same-group writes are always
/// allowed; this policy governs cross-group traffic.
/// Stored app-wide as `AppState.mcp_nonchild_write_access` and re-read by the
/// Sessions MCP host per tool call, so changes apply live.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize, Default)]
#[serde(rename_all = "lowercase")]
...
/// axis for now — browser access has no reach dimension because the browser is
/// session-scoped by construction (one isolated engine session per Unpeel
/// session).
///
/// Defaults to `On`: the engine runs an isolated per-session profile (no
/// access to the user's own logins/cookies), and Unpeel agents already run
/// with full shell access, so the sandboxed browser adds visibility rather
/// than new privilege. Settings ▸ Browser ▸ default Off is the master
/// disable; elevated future modes (shared profile, live CDP) must stay
/// opt-in per session.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize, Default)]
#[serde(rename_all = "lowercase")]
pub enum BrowserAccess {
    ...
    /// default — the browser is isolated per session, so this is visibility,
    /// not new privilege. Serialized as `"on"` for wire compatibility with
    /// pre-Ask builds.
    #[default]
    On,
}
...
/// read per tool call by the `browser` MCP domain so changes apply live.
...
pub struct BrowserSettings {
    ...
    /// "project" (default) = persistent profile per project tree under
    /// `~/.unpeel/browser/profiles/`, so a login survives across sessions of
    /// the same project — never the user's own Chrome profile; "session" =
...
pub struct AppState {
    ...
    /// App-wide policy for Sessions MCP writes outside the caller's effective
    /// group: ask the user (default), deny outright, or always allow.
    /// Same-group writes never consult this. The JSON key keeps its historical
    /// `nonchild` spelling for compatibility.
    #[serde(default)]
    pub mcp_nonchild_write_access: McpNonChildWriteAccess,
    /// User-approved cross-group write pairs, `caller session id → approved
...
mod tests {
    use super::{
        builtin_global_presets, mcp_scope_permits, normalize_pinned_sessions, normalize_projects,
        projects_in_same_tree, AppState, McpGrant, McpRole, McpScope, PinnedSidebarSession,
        Project,
    };
