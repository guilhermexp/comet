"""Shared scaffolding for the TUI's end-to-end tests.
...
class Screen:
    ...
    def sidebar_width(self):
        """Where the sidebar box ends — the first ┐ on the top row."""
        top = self.row(0)
        corner = top.find("┐")
...
class Home:
    ...
    def reserve_mobile_port(self):
        ...
        `server-port` is the *app's* rebinding contract: the TUI binds it
    ...
    def cleanup(self):
        """Kill anything a fixture parked, and any host the TUI spawned."""
...
class FakeHost:
    ...
    Lets cases assert on what the TUI *sends* a session (forwarded mouse
...
# Fake Herdr Socket API server used by integration cases.
class FakeHerdr:
    """A tiny newline-delimited Herdr Socket API server.

    The TUI integration must be testable without a running Herdr instance.
    This fake records every request and implements only the allowlisted pane
    methods the reporter is allowed to use. It deliberately returns ordinary
    Herdr response envelopes so the test covers framing and response handling,
    rather than merely accepting bytes on a socket.
    """

    def __init__(self, home, pane_id="w1:p1"):
        self.path = home.path("herdr.sock")
    ...
    def _response(self, request):
        ...
        if method == "ping":
            result = {"type": "pong", "protocol_version": 14}
        elif method == "pane.current":
            result = {"type": "pane_info", "pane": self._pane()}
        elif method == "pane.list":
            result = {"type": "pane_list", "panes": [self._pane()]}
        elif method == "pane.report_agent":
            result = {
                "type": "pane_agent_report",
                "accepted": True,
                "pane_id": self.pane_id,
            }
        elif method == "pane.release_agent":
            result = {
                "type": "pane_agent_release",
                "released": True,
                "pane_id": self.pane_id,
            }
        else:
        ...
        try:
            ...
            while not self._stop.is_set():
                ...
                try:
                    ...
                    with self._lock:
                        self._parse_errors.append((line, str(error)))
                    break
                with self._lock:
                    self._requests.append(request)
                response = json.dumps(self._response(request)).encode() + b"\n"
...
def tui_hook_port(home, exclude=(), timeout=15):
    """The port the TUI registered, ignoring any mock app's."""
    end = time.monotonic() + timeout
    while time.monotonic() < end:
...
class Case:
    ...
    def herdr(self, **kwargs):
        return self.track(FakeHerdr(self.home, **kwargs))
