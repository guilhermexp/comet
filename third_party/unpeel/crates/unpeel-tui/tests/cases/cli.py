...
def body(case):
    ...
    case.check("keys accepts escapes", run_cli(home, ["keys", short, "\\x03"]).returncode == 0)
    ...
    # A running manifest without a recorded child identity is unknowable: it
    # must fail closed rather than deleting a possibly live Session directory.
    ...
    case.check(
        "a running manifest without PID fails closed",
        crashed_resume.returncode != 0
        and f"session {crashed_id} is still running" in crashed_resume.stderr,
