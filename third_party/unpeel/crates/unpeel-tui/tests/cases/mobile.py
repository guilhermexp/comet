"""Serving a paired phone with no desktop app running — the whole point of
running `unpeel` on a headless box.

Also the polite-guest rules: the TUI never steals the phone endpoint from a
running app, and never writes the port file the app owns."""
...
def body(case):
    ...
    case.check(
        ...
        status == 200,
        "the phone keeps working across an app/TUI handover",
    )
    case.check(
    ...
    create_live = tui.wait_for(created_session_is_live, timeout=12)
    ...
    group_create_status, _ = mobile_request(
        port, "/mobile/sessions", token, method="POST",
    ...
    organization_published = tui.wait_for(bootstrap_contains_organization, timeout=12)
    case.check(
    ...
    writes_arrived = tui.wait_for(lambda: len(live_host.writes) >= 2, timeout=5)
    case.check(
    ...
    screenshot_arrived = tui.wait_for(
    ...
    replacement_live = tui.wait_for(replacement_is_live, timeout=12)
    ...
    published = tui.wait_for(archived_snapshot_contains_fixture, timeout=12)
    case.check(
    ...
    restored_published = tui.wait_for(archived_snapshot_drops_fixture, timeout=12)
    case.check(
    ...
    case.check(
        "the TUI never rewrites the app's port file",
        after == str(port),
        f"expected {port}, found {after!r} — rewriting it orphans the phone",
    )
    tui.send("q", settle=1.5)
    tui.exited()
    with open(home.path("mobile", "server-port")) as handle:
        case.check(
