"""Relay conformance: the SHIPPED phone crypto (RelayProtocol.swift,
compiled from UnpeelShared) completes a forward-secret handshake against
the Rust host through a stand-in relay, and a sealed /mobile/bootstrap
round-trips. Proves the port is byte-compatible with the client the phone
runs — the drift guard for off-LAN, exactly as `pairing` is for LAN.

Skipped with a NOTE where swiftc is absent (Linux CI)."""
...
def body(case):
    ...
    if not oracle:
        case.note("swiftc unavailable — skipped the shipped relay-crypto check")
        return
    home = case.home
    ...
    tui = case.pty(env={"UNPEEL_RELAY_URL": f"ws://127.0.0.1:{relay_port}"})
    tui.read_for(8.0)
