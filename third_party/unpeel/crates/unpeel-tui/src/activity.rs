...
//! busy/idle directly. Non-hook tools fall back to output-growth heuristics.
...
const OUTPUT_BUSY_WINDOW: Duration = Duration::from_secs(15);
...
impl ActivityEngine {
    ...
    /// Non-hook sessions: busy while the activity signal keeps changing.
    pub fn heuristic_state(
        &mut self,
        session_id: &str,
        activity_signal: u64,
        now: SystemTime,
        ...
        let entry = self.entries.entry(session_id.to_string()).or_default();
        if entry.last_signal != Some(activity_signal) {
            // First sighting is a baseline, not a change — the desktop does
            // the same (`.distantPast`), so a session discovered mid-life
            // doesn't spin for a window on discovery alone.
            let first = entry.last_signal.is_none();
            entry.last_signal = Some(activity_signal);
            if !first {
                entry.last_output_change = Some(now);
            }
        }
        match entry.last_output_change {
            Some(at) if now.duration_since(at).unwrap_or_default() < OUTPUT_BUSY_WINDOW => {
                HookState::Busy
            }
            _ => HookState::Idle,
        }
