...
mod tests {
    ...
    fn codex_notify_normalizer_maps_provider_events_before_generic_transport() {
        ...
        assert!(NOTIFY_HOOK_SCRIPT.contains("comet_task_episode"));
    }
    ...
    fn stale_tmp_claude_hooks_are_pruned() {
        let current = "/Users/me/.zeron/workers/hooks/claude-hooks.sh";
        ...
        assert!(super::is_stale_unpeel_claude_hook(
            "/private/tmp/comet-workers-menubar/hooks/claude-hooks.sh",
            current
        ));
        assert!(super::is_stale_unpeel_claude_hook(
            "/Users/me/.unpeel/hooks/claude-hooks.sh",
            current
        ));
        assert!(!super::is_stale_unpeel_claude_hook(
            "/usr/bin/echo hello",
    ...
    fn claude_hook_script_forwards_session_start_as_hook_seen() {
        ...
        assert!(super::CLAUDE_HOOK_SCRIPT.contains("comet_task_episode"));
        assert!(KIMI_HOOK_SCRIPT.contains("comet_task_episode"));

        let session_dir = temp_path("claude-record-start");
        fs::create_dir_all(&session_dir).expect("create session dir");
        fs::write(session_dir.join("comet-task-episode"), "3\n")
            .expect("write task episode");
        let stop_payload = json!({
            "hook_event_name": "Stop",
            "session_id": "provider-1"
        })
        ...
        assert_eq!(
            event.get("comet_task_episode").and_then(Value::as_u64),
            Some(3)
        );
    }
...
"#;
        ...
        assert_eq!(
            updated
                .matches("/.zeron/workers/hooks/kimi-hook.sh")
                .count(),
            9
        );
        ...
        assert!(updated.contains("BEGIN COMET MANAGED KIMI HOOKS"));
        assert!(!updated.contains("BEGIN UNPEEL MANAGED KIMI HOOKS"));
...
"#;
    ...
    fn codex_permission_request_hook_entry_matches_all_tools() {
        ...
        let legacy = command.replace("# comet-managed", "# unpeel-managed");
        assert_eq!(
            super::parse_managed_codex_hook_command(&legacy).as_deref(),
            Some(script_path.as_path())
        );
    }
    ...
    #[test]
    fn codex_hooks_keep_only_the_current_comet_managed_entry() {
        let root = temp_path("codex-hook-cleanup");
        let current = root.join(".zeron/workers/hooks/notify-hook.sh");
