...
post_hook_event() {
  ...
  hook_payload "$_hook_event_name" | curl -sS --noproxy '*' --max-time 2 -X POST -H "Content-Type: application/json" \
...
case "$EVENT_TYPE" in
  ...
  # Gemini surfaces tool-permission prompts as Notification events with
  # notification_type=ToolPermission; without this mapping a permission wait
  # shows as endless "busy" instead of the attention dot. Other notification
  # types are ignored — a broad Notification→attention mapping sticks sessions
  # yellow (see the Grok hooks rationale in grok_hooks_json).
  Notification)
    NOTIFICATION_TYPE="$(json_string_value notification_type || true)"
    case "$NOTIFICATION_TYPE" in
      ToolPermission|tool_permission) EVENT_TYPE="PermissionRequest" ;;
      *)
        printf '{}\n'
        exit 0
        ;;
...
record_last_hook_event "$EVENT_TYPE" ""
