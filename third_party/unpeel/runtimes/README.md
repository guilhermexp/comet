...
│   ├── resume.rs         # optional: resume/fresh/fork/launch identity
...
- `display`, `install`, and `suggested_presets`: generated into client-safe
  metadata so Mac, iOS, and TUI do not grow new provider tables. A custom
  `display.icon_asset` must be a package-local SVG below `assets/` and declare
  `icon_source` plus `icon_license`. Icons default to template rendering; set
...
`mcp_sessions`, `mcp_browser`, `mcp_computer`, `transcript`, `fork`,
`append_system_context`, and `notify_when_done`.
