...
pub fn install_claude_hooks() -> Result<(), String> {
    ...
    // Legacy sessions config: still referenced by launch commands of sessions
    // started before the unified server; rewritten so its exe path stays
    // current too. New launches only use the unified config.
...
pub(crate) fn claude_hook_script_path() -> PathBuf {
    app_hooks_root().join("claude-hooks.sh")
}
