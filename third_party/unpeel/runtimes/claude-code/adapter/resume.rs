use crate::resume::{
    has_resume_flag, id_in_command, join, quoted, strip_flags, strip_resume_flags, tokenize,
...
fn forked(command: &str, provider_session_id: Option<&str>) -> Option<String> {
    ...
    Some(match id {
        Some(id) => join(with_flag(
            base,
            &["--resume", &quoted(&id), "--fork-session"],
        )),
        None => join(with_flag(base, &["--continue", "--fork-session"])),
    })
}
...
mod tests {
    ...
    #[test]
    fn continue_fork_and_mint_forms_are_stable() {
        assert_eq!(
        ...
        assert_eq!(
            forked("claude", None),
            Some("claude --continue --fork-session".to_string())
        );
        assert_eq!(
            forked("claude --resume 'source-id' --model opus", None),
            Some("claude --model opus --resume 'source-id' --fork-session".to_string())
        );
