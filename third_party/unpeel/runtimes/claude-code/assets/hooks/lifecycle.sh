...
task_episode_json_field() {
  _episode_dir="${UNPEEL_SESSION_DIR:-$HOME/.unpeel/app-sessions/${UNPEEL_SESSION_ID:-}}"
  _episode_file="$_episode_dir/comet-task-episode"
  [ -r "$_episode_file" ] || return 0
  _episode=$(tr -cd '0-9' < "$_episode_file" 2>/dev/null)
  case "$_episode" in ''|*[!0-9]*) return 0 ;; esac
  printf ',"comet_task_episode":%s' "$_episode"
}
...
add_task_episode_to_payload() {
  _episode_payload="$1"
  _episode_field="$(task_episode_json_field)"
  [ -n "$_episode_field" ] || { printf '%s' "$_episode_payload"; return 0; }
  if printf '%s' "$_episode_payload" | grep -q '"comet_task_episode"[[:space:]]*:'; then
    printf '%s' "$_episode_payload"
  elif printf '%s' "$_episode_payload" | grep -q '^[[:space:]]*{'; then
    printf '%s' "$_episode_payload" | sed "1s/^[[:space:]]*{/&${_episode_field#,},/"
  else
    printf '%s' "$_episode_payload"
  fi
}
...
record_last_hook_event() {
  ...
  _record_episode="$(task_episode_json_field)"
  if [ -n "$_record_tool_name" ]; then
    _record_tool_json="$(json_escape_string "$_record_tool_name")"
    _record_payload=$(printf '{"hook_event_name":"%s","tool_name":"%s"%s%s}' "$_record_name_json" "$_record_tool_json" "$_record_generation" "$_record_episode")
  else
    _record_payload=$(printf '{"hook_event_name":"%s"%s%s}' "$_record_name_json" "$_record_generation" "$_record_episode")
  fi
...
INPUT=$(add_task_episode_to_payload "$INPUT")
