use crate::app_paths::app_hooks_root;
...
pub(crate) fn codex_notify_hook_script_path() -> PathBuf {
    app_hooks_root().join("codex-notify-hook.sh")
}
...
pub(crate) const CODEX_MANAGED_HOOK_SUFFIX: &str = "; fi # comet-managed";
pub(crate) const LEGACY_CODEX_MANAGED_HOOK_SUFFIX: &str = "; fi # unpeel-managed";
...
pub(crate) fn parse_managed_codex_hook_command(command: &str) -> Option<PathBuf> {
    let body = command
        .strip_suffix(CODEX_MANAGED_HOOK_SUFFIX)
        .or_else(|| command.strip_suffix(LEGACY_CODEX_MANAGED_HOOK_SUFFIX))?;
...
pub(crate) fn reconcile_codex_hooks_json(
    ...
    for event_name in [
        ...
        // Comet owns one canonical hook root. Remove every older managed hook,
        // even when its script still exists, while preserving foreign hooks.
        array.retain_mut(|entry| {
            ...
            entry_hooks.retain_mut(|hook| {
                ...
                if path == notify_script_path {
                    if command.ends_with(LEGACY_CODEX_MANAGED_HOOK_SUFFIX) {
                        hook["command"] = Value::String(build_codex_hook_command(notify_script_path));
                        changed = true;
                    }
                    true
                } else {
