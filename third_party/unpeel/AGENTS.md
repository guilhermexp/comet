...
> at protocol minor 6: create (from preset), Resume Agent, terminal/session
...
- `docs/agents/providers.md` — per-provider details + adding a built-in runtime
- `runtimes/README.md` — runtime package schema and contribution checklist
...
- Built-in runtime packages (descriptor, adapter, assets, provider fixtures):
  - `runtimes/<slug>/`
  - `runtimes/README.md`
...
- Shared provider transcript API:
  - `crates/unpeel-core/src/transcripts/mod.rs`
...
- `output.bin`
  - logically append-only terminal output journal with monotonic lifetime
    offsets and a physically bounded retained tail
- `session.sock`
...
- A manifest that still says `running` after its Host may have died is
  normalized to a stopped/crashed Session row only when the recorded child is
  definitively absent or its PID has been recycled. Healthy or unknown-live
  ownership fails closed: the Session stays non-resumable rather than risking
  replacement of a live terminal. A confirmed crash remains available for
  ordinary replacement **Resume**; rescan does not silently garbage-collect the
  user's recovery path.

Practical meaning:

- If the app restarts, a still-running hosted session can come back as a live terminal.
- The retained terminal output tail is replayed from `output.bin`, then live
  output resumes. Older terminal repaint bytes may have been reclaimed; the
  provider conversation/transcript remains the durable semantic history.

Caveats:

- If the hosted PTY process dies, the terminal does not survive. Reattachment
  replays on-disk output but does not revive a dead PTY. After a managed agent
  returns to a surviving shell, **Resume Agent** can continue it inside that
  same PTY; **Resume** for a stopped Session, **Restore & Resume** for a
  resumable archive, or reloading an outdated/dead terminal necessarily starts
  a new host. Plain **Restore** for an unknown/non-resumable archive does not.
  Saved output remains readable before replacement, but old terminal scrollback
  is not carried into the new Host; the provider conversation/transcript is
  what resumes. Outside those paths users resume inside the tool itself (for
  example, `claude` then `/resume`).
- If the manifest/control socket is stale, Unpeel discards that Host's liveness
  claim only after process identity proves the recorded child gone or recycled,
  then preserves the stopped/crashed Session for Resume or removal. Ambiguous
  ownership remains non-resumable.
...
(stops the host but keeps the whole session dir; **Restore & Resume** brings
back a resumable conversation, while an unknown/non-resumable command gets
plain **Restore**) and **Remove** is the destructive one — and also THE stop
verb. **Pins win over archive.** **Resume Agent** appears only after a managed
runtime exits or crashes back to its still-live shell and the Host freshly
proves that its original interactive login shell owns the foreground. A
retained expected job, a stopped/background job, a different runtime, or
unverifiable process membership suppresses the action. It then re-runs the
resume command inside the same Host/PTY while preserving Session id, socket,
`output.bin`, existing scrollback, artifacts, and grants—not shell command
history. It never stops an active runtime and is never enabled by passive
runtime observation in a blank terminal. The Host sets
disk `runtime_launch_pending` before submitting the relaunch command and mirrors
it on the wire as `runtimeLaunchPending`; both suppress a second Resume Agent
until observation or the completion wrapper resolves that launch.
**Reload Terminal** is the separate maintenance/recovery path that replaces a
Host, while stopped **Resume** and **Restore & Resume** for a resumable archive
necessarily start one. Resume planning
...
(for example host protocol → Reload Terminal, appended context → Resume Agent
once the runtime returns to its shell); add new "should restart" cases through
...
`crates/unpeel-core/src/transcripts/mod.rs`: it dispatches to runtime-package
transcript adapters that read provider-owned conversation
...
needs-input notification and deduplicates against a matching hook. Unread badges integrate with hook
...
Provider hook assets under `~/.unpeel/hooks` come from each
`runtimes/<slug>/assets/hooks/` package and are installed by its adapter at
session spawn inside the host (`session_host.rs`). Shared locked/atomic write
and reporter primitives live in `crates/unpeel-core/src/hook_assets/`.
...
- `UNPEEL_APP_PORT`
- `UNPEEL_SESSION_ID`
- `UNPEEL_RUNTIME_GENERATION` (scoped to one managed runtime invocation)
...
## Provider Hook Details and Adding a Built-in Runtime

Per-provider hook/wrapper/resume specifics live in
`docs/agents/providers.md`; the exact contribution contract is
`runtimes/README.md`. Provider-specific metadata, adapter code, assets, and
fixtures belong together under `runtimes/<slug>/`. The build discovers and
...
- `runtimes/<slug>/runtime.toml` and `adapter/`
  - provider capabilities, launch/setup, resume/context, and transcripts
- `runtimes/<slug>/assets/hooks/`
  - provider scripts, plugins, wrappers, and their package-local installer
- `crates/unpeel-core/src/integrations/mod.rs` and `hook_assets/`
  - provider-neutral dispatch, locking, atomic writes, and shared reporting
- `apps/native/UnpeelNative/Sources/UnpeelNative/HookServer.swift`
