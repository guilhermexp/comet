...
/// The overlay cards' Liquid Glass shell — same treatment as the workspace
/// open split button (WorkspaceOpenMenu.swift): real `.glassEffect` on
/// macOS 26+, frosted material + hairline fallback below.
        ...
        func walk(_ node: ProjectNode, topLevelProjectID: String) {
            ...
            where !store.archivedSessionIDs.contains(session.id) {
                ...
                order.append((
                    id: session.id,
                    working: working,
                    recency: recencyByID[session.id] ?? session.createdAt
                ))
            }
        ...
        if let projectID = store.defaultLaunchProjectID {
            ...
            for preset in [Preset.newTerminal] + store.availablePresets {
                items.append(PaletteItem(
                    ...
                    action: { [command = preset.command] in
