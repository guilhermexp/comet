...
//
//  - In-app feed: MobileRemoteServer calls `noteMobileViewer` on every
//    authenticated GET /mobile/output for a session (the iPhone's terminal
//    polling route). Those entries live in memory with a 15s TTL.
//
...
struct ViewerInfo: Identifiable, Equatable {
    enum Kind: Equatable {
        /// A paired iPhone polling through MobileRemoteServer.
        case mobile
...
final class ViewerPresenceStore: ObservableObject {
    ...
    /// sessionID → deviceID → viewer (last-write wins per device).
    private var mobileViewers: [String: [String: ViewerInfo]] = [:]
    ...
    // MARK: - In-app feed (MobileRemoteServer)

    /// Fire-and-forget entry point for non-main-actor callers (the mobile
    /// server's socket thread).
    nonisolated static func noteMobileViewerAsync(
        sessionID: String?,
        deviceID: String,
        deviceName: String?
    ) {
        ...
        Task { @MainActor in
            ViewerPresenceStore.shared.noteMobileViewer(
                sessionID: sessionID,
                deviceID: deviceID,
    ...
    func noteMobileViewer(sessionID: String, deviceID: String, deviceName: String?) {
        let viewer = ViewerInfo(
            id: "mobile:\(deviceID)",
            deviceID: deviceID,
            displayName: deviceName ?? "iPhone",
            kind: .mobile,
            lastSeen: Date()
        )
        mobileViewers[sessionID, default: [:]][deviceID] = viewer
        rebuild()
        // A phone streaming this session's output IS observing it (the Mac's
        // own "observed" gate needs the app frontmost, which it isn't while a
        // phone drives it remotely). Tell the store so it clears the unread
        // "blue dot" — mirrors the desktop clearing unread on selection.
        NotificationCenter.default.post(
            name: .unpeelMobileViewerObservedSession,
            object: nil,
    ...
    private func rebuild(now: Date = Date()) {
        ...
        for (sessionID, byDevice) in mobileViewers {
            let live = byDevice.values.filter {
                now.timeIntervalSince($0.lastSeen) <= Self.mobileEntryTTL
            }
            if !live.isEmpty { merged[sessionID] = Array(live) }
        }
        ...
        // Also drop fully-expired mobile buckets so the dictionary can't grow
        // unbounded across many short-lived sessions.
        mobileViewers = mobileViewers.compactMapValues { byDevice in
            let live = byDevice.filter {
                now.timeIntervalSince($0.value.lastSeen) <= Self.mobileEntryTTL
            }
    ...
    private func reloadPresenceFile() {
        guard let data = try? Data(contentsOf: presenceURL) else {
            // Missing/unreadable file simply means "no remote viewers".
            if !fileViewers.isEmpty { fileViewers = [:] }
            rebuild()
            return
        }
        fileViewers = Self.parsePresence(data: data)
        rebuild()
    }

    private static func parsePresence(data: Data) -> [String: [ViewerInfo]] {
