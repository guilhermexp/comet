...
// MARK: - /mcp/computer-permissions-needed + the grant-prompt alert

extension UnpeelStore {
    /// Handle POST /mcp/computer-permissions-needed: acknowledge immediately
    /// (the agent's tool call is failing right now and must not also wait on
    /// the user), then show the alert out of band. Deduplicated per missing
    /// set per app run, so a retrying agent can't spam alerts.
    func mcpComputerPermissionsNeeded(
        _ json: [String: Any]
        ...
        guard !missing.isEmpty else {
            return .failure("missing is required")
        }
        let sessionID = (json["session_id"] as? String)?
            .trimmingCharacters(in: .whitespacesAndNewlines)
        Task { @MainActor in
            self.presentComputerPermissionsNudge(missing: missing, sessionID: sessionID)
        }
        return .success(["ok": true])
    }

    /// After the user grants computer access to a session (Ask alert), check
    /// the TCC grants right away: the user is engaged, and without them the
    /// approval they just gave leads straight into a failing first action.
