...
/// their first hook event; everything else stays on the output heuristic in
/// UnpeelStore.scanSessions.
@MainActor
final class SessionActivityEngine {
    ...
    /// produced a hook event (caller falls back to output heuristics).
...
struct LastHookEvent {
    ...
    static func parse(_ data: Data) -> LastHookEvent? {
        ...
        let name = HookServer.normalizedHookEventName(rawName)
        let latchOnly: Bool
        switch name {
        ...
        return LastHookEvent(
            hookEventName: name,
            latchOnly: latchOnly,
            runtimeGeneration: HookServer.runtimeGeneration(from: json)
        )
    }
}
