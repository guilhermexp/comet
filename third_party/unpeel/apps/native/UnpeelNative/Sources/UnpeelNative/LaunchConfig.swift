...
enum LaunchConfig {
    ...
    static var attachBinary: String {
        ...
        let candidates = [
            "\(repoRoot)/apps/native/unpeel-attach/target/release/unpeel-attach",
            "\(repoRoot)/apps/native/unpeel-attach/target/debug/unpeel-attach",
        ]
    ...
    /// `direct:` makes Ghostty exec the binary instead of wrapping it in
    /// `login(1)`, which would print the "Last login"/"You have mail"
    /// banner above every attach.
    static func attachCommand(sessionID: String) -> String {
        ...
        // Pass the dir as an explicit argv flag: argv survives the login(1)
        // wrapping/env scrubbing the surface command can go through, and it
        // works with attach binaries that predate UNPEEL_HOME support. The
        // env(1) injection stays as well for anything attach spawns.
