...
final class RestartGhostTests: XCTestCase {
    ...
    func testForkKeepsBothSessions() {
        // Fork gets a *fresh* created_at, so the original and the fork never
        // collide — neither is a ghost even though one may later exit.
        let ghosts = UnpeelStore.supersededRestartGhostIDs([
            Candidate(id: "source", projectID: "p", createdAt: 1000, isLive: true),
            Candidate(id: "fork", projectID: "p", createdAt: 2000, isLive: true),
        ])
        XCTAssertTrue(ghosts.isEmpty)
    }
