...
final class HostedSessionManifestTests: XCTestCase {
    func testAppendedContextLockPathMatchesRustContract() {
        let root = URL(fileURLWithPath: "/tmp/unpeel-test", isDirectory: true)
        XCTAssertEqual(
            UnpeelStore.appendedContextLockURL(
                unpeelDir: root,
                sessionID: "session"
            ).path,
            "/tmp/unpeel-test/session-appended-context-locks/" +
                "3f3af1ecebbd1410ab417ec0d27bbfcb5d340e177ae159b59fc8626c2dfd9175.lock"
        )
    }
    ...
    @MainActor
    func testPendingContextWaitsForActiveRuntimeBeforeOfferingSafeAction() throws {
        func manifest(
            hostProtocolVersion: Int?,
            activeRuntimeID: String?
        ) throws -> HostedSessionManifest {
            var root: [String: Any] = [
                "session": [
                    "id": "session",
                    "project_id": "project",
            ...
            if let hostProtocolVersion {
                root["host_protocol_version"] = hostProtocolVersion
            }
            if let activeRuntimeID {
                root["runtime"] = [
                    "currentObservation": ["id": activeRuntimeID],
                ]
            }
        ...
        for version in [2, 3, 4] {
            let active = try manifest(
                hostProtocolVersion: version,
                activeRuntimeID: "claude"
            )
            let activeRecommendation = UnpeelStore.restartRecommendation(
                for: active,
                pendingAppendSystemContext: "Use the new policy"
            )
            XCTAssertEqual(
                activeRecommendation?.message,
                "Appended system context will apply the next time the agent is resumed."
            )
            XCTAssertNil(activeRecommendation?.action)
        }

        let returnedShell = try manifest(
            hostProtocolVersion: 3,
            activeRuntimeID: nil
        )
        XCTAssertEqual(
            UnpeelStore.restartRecommendation(
                for: returnedShell,
                pendingAppendSystemContext: "Use the new policy"
            )?.action,
            .resumeAgent
        ...
        let unknownHost = try manifest(hostProtocolVersion: nil, activeRuntimeID: nil)
        XCTAssertNil(UnpeelStore.restartRecommendation(
            for: unknownHost,
            pendingAppendSystemContext: nil
        ))

        let maintenanceHost = try manifest(
    ...
    func testRestartContextReconciliationPreservesConcurrentNextIntent() {
        let pending = ["session": "old", "other": "untouched"]
        XCTAssertEqual(UnpeelStore.reconciledPendingAppendedContexts(
            pending,
            sessionID: "session",
            currentSharedContext: "new concurrent context"
        ), ["session": "new concurrent context", "other": "untouched"])
        XCTAssertEqual(UnpeelStore.reconciledPendingAppendedContexts(
            pending,
            sessionID: "session",
            currentSharedContext: nil
        ), ["other": "untouched"])
    }
