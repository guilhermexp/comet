...
final class TerminalSurfaceCoordinator {
    ...
    init() {
        ...
        activityRelay.coordinator = self
    ...
    /// open, a real display link actively pumps `ghostty_app_tick` and
    /// draws every frame, pulling whatever the core queued instead of
    /// waiting to be pushed. The window re-arms on every render request
    /// and on user scroll input, and the relay gate keeps closed-window
    /// frames nearly free.
    private let activityLink = DisplayLink()
    private let activityRelay = TerminalActivityLinkRelay()
    private var activityWindowDeadline: TimeInterval = 0
    private static let activityWindowDuration: TimeInterval = 1.0
    ...
    func noteRenderActivity() {
        activityWindowDeadline = Self.monotonicTimestamp() + Self.activityWindowDuration
        activityRelay.setActive(true)
    }

    func activityFrame(context: DisplayLinkCallbackContext) {
        guard canRenderFrame else {
            activityRelay.setActive(false)
            return
        }
        guard Self.monotonicTimestamp() <= activityWindowDeadline else {
            activityRelay.setActive(false)
            return
        }
    ...
    func rebuildIfReady(removingBridgeFrom previousController: TerminalController? = nil) {
        ...
        // every host write so remote output presents at display rate.
        configuration.inMemorySession?.onHostBytes = { [weak self] in
            if Thread.isMainThread {
                MainActor.assumeIsolated { self?.noteRenderActivity() }
            } else {
                DispatchQueue.main.async { self?.noteRenderActivity() }
            }
        }
...
/// Hops display-link frames onto the main actor while the coordinator's
/// activity window is open. `synchronization` fires on the display-link
/// thread every frame for the lifetime of the link; the lock-guarded gate
/// keeps closed-window frames to a single lock acquisition with no
/// main-thread work.
final class TerminalActivityLinkRelay: DisplayLinkDelegate, @unchecked Sendable {
    ...
    func synchronization(context: DisplayLinkCallbackContext) {
        ...
        DispatchQueue.main.async { [weak self] in
            MainActor.assumeIsolated {
                self?.coordinator?.activityFrame(context: context)
            }
        }
    }
}
