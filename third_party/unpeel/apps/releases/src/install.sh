...
#
# Installs the `unpeel` terminal UI plus its `unpeel-host` sibling (the TUI
# spawns sessions through it — resolve_host_binary looks for a sibling first).
...
if ! curl -fsSL -o "$tmp/unpeel.tar.gz" "$url"; then
  ...
  echo "       building from source: cargo build --release -p unpeel-tui -p unpeel-host" >&2
  exit 1
fi
...
install -m 755 "$tmp/unpeel" "$tmp/unpeel-host" "$dir"

# Record where this install came from: the TUI's update check reads the
...
echo ""
echo "Start it:"
echo "  unpeel          open the terminal UI (first run sets up presets + projects)"
echo "  unpeel --help   every command and flag"
echo ""
echo "Best in Ghostty (https://ghostty.org) — the same terminal engine Unpeel's"
echo "own surfaces use. Any modern terminal works."
echo ""
echo "Docs: https://unpeel.com/docs/terminal-ui"
