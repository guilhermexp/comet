...
/// refresh apps/relay/test/relay-kat-vectors.json after an intentional change).
final class RelayCryptoVectorTests: XCTestCase {
    ...
    // to apps/relay/test/relay-kat-vectors.json, which the WebCrypto KAT
    // reproduces independently — so a match proves Swift and JS agree byte for
    // byte. Regenerate with UNPEEL_EMIT_RELAY_KAT=1 on an intentional change.
