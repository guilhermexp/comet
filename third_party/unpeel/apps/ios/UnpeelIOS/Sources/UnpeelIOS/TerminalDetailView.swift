...
enum TerminalChrome {
    static let backgroundHex: UInt32 = 0x1A1A1F
...
struct TerminalDetailView: View {
    ...
    /// The selected session's resolved terminal background, so the whole
    /// screen (incl. safe-area edges) matches the provider TUI's theme.
    private var chromeBackground: Color {
        store.selectedSession?.terminalBackgroundHex
    ...
    var body: some View {
        ZStack {
            TerminalBackground(color: chromeBackground)
        ...
        #if os(iOS)
        // Reveal the sessions sidebar by dragging rightward from anywhere in
        // the LEFT HALF of the screen (not just the extreme edge). Simultaneous
        // + commit-on-release so it never blocks terminal taps/scroll: it only
        // fires when the drag STARTS in the left region, ends decisively
        // rightward, and is clearly horizontal.
        .simultaneousGesture(sidebarRevealDrag)
        // Mirror the auto-clearing peek into the store so the drawer overlay
        // (a sibling view) can render it — and, crucially, so it always
        // returns to nil when the gesture ends/cancels.
        .onChange(of: sidebarRevealGesture) { newValue in
            if store.sidebarDragReveal != newValue { store.sidebarDragReveal = newValue }
        }
        #endif
    ...
    #if os(iOS)
    /// How far in from the left edge a reveal drag may start — roughly the
    /// left half of the screen, so it's easy to catch without living at the
    /// extreme edge.
    private static let sidebarRevealStartMaxX: CGFloat = 220

    private var sidebarRevealDrag: some Gesture {
        DragGesture(minimumDistance: 18, coordinateSpace: .local)
            .updating($sidebarRevealGesture) { value, state, _ in
                ...
                if value.startLocation.x < Self.sidebarRevealStartMaxX,
                   horizontal, value.translation.width > 0 {
            ...
            .onEnded { value in
                ...
                let horizontal = abs(value.translation.width)
                    > abs(value.translation.height) * 1.3
                guard value.startLocation.x < Self.sidebarRevealStartMaxX,
                      horizontal, value.translation.width > 0
                else { return }
...
private enum SessionDetailLayout {
    static let terminalTopInset: CGFloat = 116
    static let terminalLandscapeTopInset: CGFloat = 68
    static let terminalBottomInset: CGFloat = 44
}
...
struct TerminalTopBar: View {
    ...
    var body: some View {
        ZStack {
            ...
            .accessibilityHint("Hold to rename or pin this session")
            ...
            HStack(spacing: 10) {
                ...
                // Always shown so it's always openable as a session switcher —
                // not gated on there being active/unread sessions (that made
                // the bell vanish when nothing was running). Spins while any
                // agent works anywhere; otherwise a plain bell.
                    ...
                    TimelineView(.periodic(from: .now, by: 60)) { _ in
                        Text(
                            blocked
                                ...
                                : working
                                    ? "run"
                                    : RelativeAge.shortString(
                                        fromUnixMs: session.updatedAtUnixMs
                                            ?? session.createdAtUnixMs
                                    )
                        )
                            .font(.system(size: 10, weight: .medium))
                            .foregroundStyle(
                                blocked ? IOSSidebarTheme.attention : .white.opacity(0.42)
                            )
                    }
...
struct EmptyTerminalState: View {
    ...
    var body: some View {
        VStack(spacing: 16) {
            UnpeelBrandLogo(size: 72)
                .padding(.bottom, 4)
            Image(systemName: disconnected ? "wifi.slash" : "terminal")
