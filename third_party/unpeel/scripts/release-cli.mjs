#!/usr/bin/env node
// Build and publish the Unpeel CLI (`unpeel` + `unpeel-host`) tarballs that
...
const appDir = resolve(repoRoot, 'apps/website')
const cratesDir = resolve(repoRoot, 'crates')
const wranglerConfig = readFileSync(resolve(appDir, 'wrangler.jsonc'), 'utf8')
const configAccountId = wranglerConfig.match(/"account_id"\s*:\s*"([^"]+)"/)?.[1]
...
const bucket = String(args.bucket ?? process.env.UNPEEL_RELEASE_BUCKET ?? 'unpeel-releases')
...
if (args['macos-universal']) {
  ...
  for (const triple of triples) {
    run('cargo', [
      ...
      '-p', 'unpeel-tui', '-p', 'unpeel-host',
      '--target', triple
    ], { env: rustEnv })
  }
  const stage = mkdtempSync(resolve(tmpdir(), 'unpeel-cli-'))
  for (const bin of ['unpeel', 'unpeel-host']) {
    const out = resolve(stage, bin)
    run('lipo', [
      '-create', '-output', out,
      ...triples.map((t) => resolve(cratesDir, 'target', t, 'release', bin))
    ])
...
for (const [target, file] of Object.entries(tarballs)) {
  ...
  try {
    const extraction = spawnSync('tar', [
      ...
      'unpeel', 'unpeel-host', 'BUILD_PROVENANCE.json'
    ], { encoding: 'utf8' })
