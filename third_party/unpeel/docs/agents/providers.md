...
the detached Host starts and again before the provider PTY starts. Keep that
rule provider-independent: installed provider-specific Herdr hooks must not
claim or race the TUI's aggregate `custom:unpeel` authority. See
...
- Does not advertise Fork or Append system context: Kimi's `/fork` is
  interactive-only, and its agent-file mechanism replaces rather than appends
...
- Does not advertise Fork or Append system context: `/fork` is interactive and
  `--system` replaces the base prompt. Cline also exposes no approval-request
  hook, so custom `--auto-approve false` prompts have no distinct
  hook-driven attention state
- Full findings: `docs/feature/cline-cli-integration.md`

Pi:

- No hook-port integration today; Unpeel relies on output heuristics
...
- Does not advertise Fork or Append system context: no fork primitive, and the
  only prompt mechanisms are replace-mode settings (`run.system_prompt`) or
  unstable eval env vars. Sessions/Browser MCP injection is deferred: muse's
...
7. Run `bun run generate:runtimes` and `bun run check:runtimes`, then the full
   core/Host/TUI/native/iOS suites. Deep integrations require a real hosted-PTY
