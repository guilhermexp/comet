...
signs, and launches that exact bundle with `open -n`, so an installed app with
the shared bundle id cannot steal the launch). They are visually distinct: **"Unpeel Dev"** in the
...
builds land in the *same* `dist/Unpeel.app` path: after any `bun run release`
...
> operational repo (`~/Dev/unpeel-account`, the sibling checkout).
...
agent superpowers on by default. CLI detection and installs live in
Settings ▸ Presets (Rescan PATH + the "Not installed" install rows).
