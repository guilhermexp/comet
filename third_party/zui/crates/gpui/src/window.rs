...
impl Window {
    ...
    /// Sets the WindowServer background blur radius in logical points, or
    /// `None` for the platform default. Only has an effect while the background
    /// appearance is [`WindowBackgroundAppearance::Blurred`].
    ///
    /// The macOS implementation forwards the value verbatim to
    /// `CGSSetWindowBackgroundBlurRadius` without applying the display scale
    /// factor. How the WindowServer interprets that number is private AppKit
    /// behavior, not something this tree observes. Applying the scale factor
    /// would change the appearance of every existing consumer, so it must stay
    /// a deliberate decision rather than a silent one.
    pub fn set_background_blur_radius(&self, radius: Option<Pixels>) {
        self.platform_window.set_background_blur_radius(radius);
    }
