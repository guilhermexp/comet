...
const LEGACY_WINDOW_SERVER_BLUR_RADIUS: i64 = 80;
const DECLARED_WINDOW_SERVER_BLUR_RADIUS_MAX: f32 = 128.0;

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
enum WindowBackgroundBlurMode {
    None,
    Material,
    WindowServer { radius: i64 },
}

/// Decides how a window's blurred background is produced.
///
/// `declared_radius` is in logical points and is an ordered quantity, so it is
/// clamped into `0..=DECLARED_WINDOW_SERVER_BLUR_RADIUS_MAX`; infinities clamp
/// to the corresponding bound rather than escaping it. `NaN` is not ordered
/// against that range and is therefore treated as no declaration at all,
/// falling back to the platform default.
///
/// `is_modern_appkit` selects which of the two blur implementations provides
/// that default. Whether `-[NSVisualEffectView respondsToSelector:
/// @selector(_updateProxyLayer)]`: on macOS Catalina/Big Sur
/// `NSVisualEffectView` doesn't own concrete sublayers but uses a
/// `CAProxyLayer`, so the legacy path drives the WindowServer API directly. On
...
fn window_background_blur_mode(
    ...
    is_modern_appkit: bool,
) -> WindowBackgroundBlurMode {
    if appearance != WindowBackgroundAppearance::Blurred {
        return WindowBackgroundBlurMode::None;
    }

    let declared_radius = declared_radius.filter(|radius| !radius.as_f32().is_nan());
    match declared_radius {
        Some(radius) => WindowBackgroundBlurMode::WindowServer {
            radius: radius
                .as_f32()
                .clamp(0.0, DECLARED_WINDOW_SERVER_BLUR_RADIUS_MAX)
                .round() as i64,
        },
        None if is_modern_appkit => WindowBackgroundBlurMode::Material,
        None => WindowBackgroundBlurMode::WindowServer {
            radius: LEGACY_WINDOW_SERVER_BLUR_RADIUS,
        },
    }
}
...
impl MacWindowState {
    ...
    fn apply_background_appearance(&mut self) {
        let background_appearance = self.background_appearance;
        let opaque = background_appearance == WindowBackgroundAppearance::Opaque;
        self.renderer.update_transparency(!opaque);

        unsafe {
            self.native_window.setOpaque_(opaque as BOOL);
            ...
            self.native_window.setBackgroundColor_(background_color);

            let is_modern_appkit = NSAppKitVersionNumber >= NSAppKitVersionNumber12_0;
            let mode = window_background_blur_mode(
            ...
            match mode {
                WindowBackgroundBlurMode::None => {
                    ...
                    if should_clear_window_server {
                        ...
                        self.window_server_blur_applied = false;
                    }
                }
                WindowBackgroundBlurMode::Material => {
                    ...
                    if self.blurred_view.is_none() {
                        ...
                        self.blurred_view = Some(blur_view.autorelease());
                    }
                }
                WindowBackgroundBlurMode::WindowServer { radius } => {
                    ...
                    self.window_server_blur_applied = true;
                }
            }
        }
    }
...
impl PlatformWindow for MacWindow {
    ...
    fn set_background_appearance(&self, background_appearance: WindowBackgroundAppearance) {
        ...
        this.apply_background_appearance();
    }

    fn set_background_blur_radius(&self, radius: Option<Pixels>) {
        let mut this = self.0.as_ref().lock();
        this.background_blur_radius = radius;
        this.apply_background_appearance();
    }
...
mod tests {
    ...
    #[test]
    fn window_blur_non_blurred_appearance_has_no_blur() {
        assert_eq!(
            window_background_blur_mode(WindowBackgroundAppearance::Opaque, Some(px(24.0)), true),
            WindowBackgroundBlurMode::None
        );
        assert_eq!(
            window_background_blur_mode(
    ...
    #[test]
    fn window_blur_blurred_without_radius_uses_material_on_modern_appkit() {
        assert_eq!(
            window_background_blur_mode(WindowBackgroundAppearance::Blurred, None, true),
            WindowBackgroundBlurMode::Material
        );
        assert_eq!(
            window_background_blur_mode(
    ...
    #[test]
    fn window_blur_blurred_without_radius_uses_window_server_80_on_legacy_appkit() {
        assert_eq!(
            window_background_blur_mode(WindowBackgroundAppearance::Blurred, None, false),
            WindowBackgroundBlurMode::WindowServer { radius: 80 }
        );
    }

    #[test]
    fn window_blur_blurred_with_declared_radius_uses_clamped_window_server() {
        assert_eq!(
            window_background_blur_mode(WindowBackgroundAppearance::Blurred, Some(px(24.0)), true),
            WindowBackgroundBlurMode::WindowServer { radius: 24 }
        );
        assert_eq!(
            window_background_blur_mode(WindowBackgroundAppearance::Blurred, Some(px(80.0)), true),
            WindowBackgroundBlurMode::WindowServer { radius: 80 }
        );
        assert_eq!(
            window_background_blur_mode(WindowBackgroundAppearance::Blurred, Some(px(1e9)), true),
            WindowBackgroundBlurMode::WindowServer { radius: 128 }
        );
        assert_eq!(
            window_background_blur_mode(
                WindowBackgroundAppearance::Blurred,
        ...
        assert_eq!(
            window_background_blur_mode(WindowBackgroundAppearance::Blurred, Some(px(-5.0)), false),
            WindowBackgroundBlurMode::WindowServer { radius: 0 }
        );
        assert_eq!(
            window_background_blur_mode(
                WindowBackgroundAppearance::Blurred,
                Some(px(f32::NEG_INFINITY)),
                false
            ),
            WindowBackgroundBlurMode::WindowServer { radius: 0 }
        );
